...
namespace Studio {
    ...
    bool Intellesense::activate(QString &text, int row, int &cursorIndex, QKeyEvent* keyEvent,
		...
		const boost::optional<std::vector<ARL::ScriptAnalyzer::IntellesenseResult> >& result)
    {
    ...
    bool Intellesense::isEnumerator(QString previousWord)
    {
        ...
        {
            for (std::vector<const ARL::Reflection::EnumDescriptor*>::const_iterator i = ARL::Reflection::EnumDescriptor::enumsBegin(); 
                 i != ARL::Reflection::EnumDescriptor::enumsEnd();
                 ++i)
            {
				ARL::Reflection::Metadata::Item* metadata = ARL::Reflection::Metadata::Reflection::singleton()->get(**i);
				if (ARL::Reflection::Metadata::Item::isDeprecated(metadata, **i))
				{
					continue;
				}
    ...
    int Intellesense::populateItemLists(QString text, int row, int index, bool searchDatamodel, const boost::optional<std::vector<ARL::ScriptAnalyzer::IntellesenseResult> >& result)
    {
		...
		shared_ptr<ARL::LuaSourceContainer> script = shared_ptr<ARL::LuaSourceContainer>();
	...
	QString Intellesense::getCurrentScriptPath()
	{
		...
		{
			shared_ptr<ARL::Instance> currentInstance = scriptDoc->getCurrentScript().toInstance();
	...
	shared_ptr<ARL::LuaSourceContainer> Intellesense::getCurrentScript()
	{
		if (RobloxScriptDoc* scriptDoc = dynamic_cast<RobloxScriptDoc *>(RobloxDocManager::Instance().getCurrentDoc()))
		{
			return scriptDoc->getCurrentScript().toInstance();
		}
		return shared_ptr<ARL::LuaSourceContainer>();
	}

    template<bool includeType>
    static void catenateArg(int& count, QString& text, const ARL::Reflection::SignatureDescriptor::Item& item)
    {
        if (count>0)
            text += ", ";
        if (includeType)
    ...
    void Intellesense::populateProperty(const ARL::Reflection::PropertyDescriptor* d)
    {
        if (!d || !d->isPublic() || !d->isScriptable())
            return;

        ARL::Reflection::Metadata::Item* metadata = ARL::Reflection::Metadata::Reflection::singleton()->get(*d);
    ...
    void Intellesense::populateFunction(const ARL::Reflection::FunctionDescriptor* d)
    {
        if (!d)
            return;
        ...
        QString toolTip = "";
        const ARL::Reflection::SignatureDescriptor& signature = d->getSignature();
    ...
    void Intellesense::populateYieldFunction(const ARL::Reflection::YieldFunctionDescriptor* d)
    {
        if (!d)
            return;
    ...
    void Intellesense::populateEvent(const ARL::Reflection::EventDescriptor* d)
    {
        if (!d || !d->isPublic() || !d->isScriptable())
            return;
    ...
    void Intellesense::populateCallBack(const ARL::Reflection::CallbackDescriptor* d)
    {
        if (!d)
            return;
    ...
    void Intellesense::populateEnumerator(const ARL::Reflection::EnumDescriptor::Item* d)
    {
		ARL::Reflection::Metadata::Item* metadata = ARL::Reflection::Metadata::Reflection::singleton()->get(*d);
		if (ARL::Reflection::Metadata::Item::isDeprecated(metadata, *d))
		{
			return;
		}
	...
	int Intellesense::repopulateUserVariables(const std::vector<ARL::ScriptAnalyzer::IntellesenseResult>& result, unsigned int row, unsigned int column)
	{
    ...
    bool Intellesense::populateItemListsFromVariant(const ARL::Reflection::Variant varValue, bool showAllChildren)
    {
        if(varValue.isType<boost::shared_ptr<ARL::Instance> >())
        {
            boost::shared_ptr<ARL::Instance> pInstance = varValue.get<boost::shared_ptr<ARL::Instance> >();

            std::for_each(
                pInstance->getDescriptor().begin<ARL::Reflection::PropertyDescriptor>(), 
                pInstance->getDescriptor().end<ARL::Reflection::PropertyDescriptor>(),
                boost::bind(&Intellesense::populateProperty, this, _1)
                );

            std::for_each(
                pInstance->getDescriptor().begin<ARL::Reflection::FunctionDescriptor>(), 
                pInstance->getDescriptor().end<ARL::Reflection::FunctionDescriptor>(),
                boost::bind(&Intellesense::populateFunction, this, _1)
                );

            std::for_each(
                pInstance->getDescriptor().begin<ARL::Reflection::YieldFunctionDescriptor>(), 
                pInstance->getDescriptor().end<ARL::Reflection::YieldFunctionDescriptor>(),
                boost::bind(&Intellesense::populateYieldFunction, this, _1)
                );

            std::for_each(
                pInstance->getDescriptor().begin<ARL::Reflection::EventDescriptor>(), 
                pInstance->getDescriptor().end<ARL::Reflection::EventDescriptor>(),
                boost::bind(&Intellesense::populateEvent, this, _1)
                );

            std::for_each(
                pInstance->getDescriptor().begin<ARL::Reflection::CallbackDescriptor>(), 
                pInstance->getDescriptor().end<ARL::Reflection::CallbackDescriptor>(),
                boost::bind(&Intellesense::populateCallBack, this, _1)
                );

            if (pInstance->getChildren())
            {
                ARL::Instances::const_iterator iter = pInstance->getChildren()->begin();
        ...
        else if (varValue.isType<ARL::Vector2>())
            return repopulateItemMetadata("Vector2");
        else if (varValue.isType<ARL::Vector2int16>())
            return repopulateItemMetadata("Vector2int16");
        else if (varValue.isType<ARL::Vector3>())
            return repopulateItemMetadata("Vector3");
        else if (varValue.isType<ARL::Vector3int16>())
        ...
        else if (varValue.isType<ARL::Region3>())
            return repopulateItemMetadata("Region3");
        else if (varValue.isType<ARL::Region3int16>())
            return repopulateItemMetadata("Region3int16");
        else if (varValue.isType<ARL::BrickColor>())
        ...
        else if (varValue.isType<ARL::UDim2>())
            return repopulateItemMetadata("UDim2");
		else if (varValue.isType<ARL::Rect2D>())
			return repopulateItemMetadata("Rect");
		else if (varValue.isType<ARL::PhysicalProperties>())
			return repopulateItemMetadata("PhysicalProperties");
        else if (varValue.isType<shared_ptr<const ARL::Reflection::ValueTable> >())
        {
            shared_ptr<const ARL::Reflection::ValueTable> valMap = varValue.get<shared_ptr<const ARL::Reflection::ValueTable> >();

            for (ARL::Reflection::ValueTable::const_iterator iter = valMap->begin(); iter != valMap->end(); ++iter)
                addProperty(iter->first.c_str());

            return true;
        }
        else if (varValue.isType<shared_ptr<const ARL::Reflection::ValueArray> >())
        {
