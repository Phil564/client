...
void ScriptComboBox::setCommandHistory(const QStringList& commandHistory)
{
    ...
    if (!m_CommandHistory.isEmpty())
    {
        for (int i = 0; i < m_CommandHistory.size(); ++i)
        {
            ...
            if (ARL::ScriptContext::checkSyntax(command.toStdString(), errorLine, errorMessage))
                addItem(command);
        }
        setCurrentIndex(-1);
    }
...
void ScriptComboBox::onScriptInputReturnPressed()
{
    ...
    {
        ARL::StandardOut::singleton()->printf(ARL::MESSAGE_ERROR, "Command cannot be executed without an open place. Please open a place and try again.");
        return;
    }
    ...
    if (!toExecute.isEmpty())
    {
        int errorLine = 0;
        std::string errorMessage;
        if (ARL::ScriptContext::checkSyntax(toExecute.toStdString(), errorLine, errorMessage))
        {
        ...
        {
            ARL::StandardOut::singleton()->printf(ARL::MESSAGE_ERROR, "Error in script: %s", errorMessage.c_str());
