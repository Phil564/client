...
{
	std::string robloxUserId = ARL::StringConverter<int>::convertToString(RobloxUser::singleton().getUserId());
...
void fillPublishFeatures(shared_ptr<ARL::Instance> instance, PublishFeatures* publishFeatures)
{
	shared_ptr<ARL::Script> script = ARL::Instance::fastSharedDynamicCast<ARL::Script>(instance);
	if(ARL::Instance::fastDynamicCast<ARL::Script>(instance.get()) || ARL::Instance::fastDynamicCast<ARL::ModuleScript>(instance.get()))
	{
		std::string code;
		try
		{
	...
	{
		shared_ptr<ARL::RemoteFunction> remoteFunction = ARL::Instance::fastSharedDynamicCast<ARL::RemoteFunction>(instance);
		shared_ptr<ARL::RemoteEvent> remoteEvent = ARL::Instance::fastSharedDynamicCast<ARL::RemoteEvent>(instance);

		if(remoteFunction || remoteEvent)
			publishFeatures->hasRemoteFunctions = true;
	}
	
	shared_ptr<ARL::PartInstance> partInstance = ARL::Instance::fastSharedDynamicCast<ARL::PartInstance>(instance);
	if(partInstance)
		publishFeatures->partCount++;
}

void StudioAnalytics::reportPublishStats(ARL::DataModel* dm)
{
