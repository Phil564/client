...
{
    ARL::PluginManager::singleton()->setStudioHost(this);
...
void RobloxPluginHost::setNotifier(ARL::IHostNotifier *notifier)
{ 
...
void RobloxPluginHost::onShowToolbar(int toolbarID, bool show)
{
    if ( !m_ToolBarMap.contains(toolbarID) && !m_RibbonGroupMap.contains(toolbarID) )
	{
        ...
        if ( show )
            ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO,"Plugin attempting to show toolbar that does not exist - %i",toolbarID);
        return;
	}
...
void RobloxPluginHost::onEnableToolbar(int toolbarID, bool enable)
{
    if ( !m_ToolBarMap.contains(toolbarID) && !m_RibbonGroupMap.contains(toolbarID) )
	{
		// only output an error if we're showing a toolbar that is supposed to exist
        if ( enable )
            ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO,"Plugin attempting to disable/enable toolbar that does not exist - %i",toolbarID);
        return;
	}
...
void RobloxPluginHost::deleteToolbars(const std::vector<void*>& toolbars)
{
    ...
    for ( size_t i = 0 ; i < toolbars.size() ; ++i ) 
    {
		...
		if ( mainWindow.isRibbonStyle() )
		{
            ...
            {
                ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO,"Plugin attempting to delete toolbar that does not exist - %i",toolbarID);
                continue;
            }
...
void RobloxPluginHost::setSetting(int assetId, const std::string& key,
	const ARL::Reflection::Variant& value)
{
    shared_ptr<ARL::Reflection::ValueTable> values(new ARL::Reflection::ValueTable());
...
void RobloxPluginHost::onCreateToolbar(int toolbarID,QString name)
{
	...
	if ( mainWindow.isRibbonStyle() )
	{
		ARLASSERT(!m_RibbonGroupMap.contains(toolbarID));
	...
	{
        ARLASSERT(!m_ToolBarMap.contains(toolbarID));
...
void RobloxPluginHost::onCreateButton(int toolbarID,int actionID,QString text,QString tooltip,QString iconName)
{
    ...
    {
        ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO,"Plugin attempting to add button to toolbar that does not exist - %i, %i",toolbarID,actionID);
        return;
    }
...
void RobloxPluginHost::loadPlugins(ARL::DataModel* dataModel)
{
    if ( !dataModel )
        return;
	...
	ARL::PluginManager::singleton()->startModelPluginScripts(dataModel);
}

void RobloxPluginHost::loadPluginsFromPath(ARL::DataModel* dataModel,const QString &srcPath)
{
...
void RobloxPluginHost::loadPluginsInternal(ARL::DataModel* dataModel,const QString& srcPath)
{
    QDir srcDir(srcPath);    
...
void RobloxPluginHost::doLoadPlugin(ARL::DataModel* dataModel, QString filename)
{
    boost::mutex::scoped_lock lock(ARL::PluginManager::singleton()->mutex);
    ...
    try
    {        
        ARL::DataModel::LegacyLock legacylock(dataModel, ARL::DataModelJob::Write);
        boost::shared_ptr<ARL::ScriptContext> pContext = ARL::shared_from(dataModel->create<ARL::ScriptContext>());
        if (pContext)    
            pContext->executeInNewThread(ARL::Security::StudioPlugin,
                ARL::ProtectedString::fromTrustedSource(c_strToExe), c_strFileName);
    }
...
void RobloxPluginHost::findAndLoadModelPlugins(ARL::DataModel* dataModel, const QString& srcPath)
{
...
void RobloxPluginHost::loadInstalledModelPlugins(ARL::DataModel* dataModel)
{
	...
	if (!pluginMetadataString.isEmpty() && !ARL::WebParser::parseJSONTable(pluginMetadataString.toUtf8().constData(), values))
	{
		ARL::StandardOut::singleton()->print(ARL::MESSAGE_ERROR,
	...
	for (ARL::Reflection::ValueTable::const_iterator itr = values->begin();
		itr != values->end(); ++itr)
	{
		...
		shared_ptr<const ARL::Reflection::ValueTable> valueTable =
			itr->second.cast<shared_ptr<const ARL::Reflection::ValueTable> >();
		ARL::Reflection::ValueTable::const_iterator enabledData = valueTable->find(kEnabledJsonKey);
...
void RobloxPluginHost::doLoadModelPlugin(ARL::DataModel* dataModel,
...
{	
	...
	try
	{
		...
		if (ARL::ContentProvider* cp = dataModel->create<ARL::ContentProvider>())
		{
			ARL::LuaSourceContainer::blockingLoadLinkedScriptsForInstances(cp, instances);
		}

		ARL::PluginManager::singleton()->addModelPlugin(dataModel, instances, assetId);
	}
	...
	std::string* serialized, std::exception* error, boost::function<void(bool)> resultCallback)
{
	...
	// Update Metadata
	{
		...
		if (readPluginMetadata(metadata))
		{
			...
			ARL::Reflection::ValueTable::iterator pluginData = metadata->find(assetIdStdString);
			shared_ptr<ARL::Reflection::ValueTable> newData;
			...
			(*metadata)[assetIdStdString] = shared_ptr<const ARL::Reflection::ValueTable>(newData);

			writePluginMetadata(metadata);
		}
	}

	ARL::StandardOut::singleton()->print(ARL::MESSAGE_INFO,
...
void RobloxPluginHost::setPluginEnabled(int assetId, bool enabled)
{
	...
	if (readPluginMetadata(metadata))
	{
		ARL::Reflection::ValueTable::iterator itr = metadata->find(assetIdString);
		if (itr == metadata->end())
		{
			ARL::StandardOut::singleton()->printf(ARL::MESSAGE_ERROR,
		...
		shared_ptr<ARL::Reflection::ValueTable> newData(
			new ARL::Reflection::ValueTable(
				*(itr->second.cast<shared_ptr<const ARL::Reflection::ValueTable> >())));
		(*newData)[kEnabledJsonKey] = enabled;
		(*metadata)[assetIdString] = shared_ptr<const ARL::Reflection::ValueTable>(newData);
...
bool RobloxPluginHost::readPluginMetadata(shared_ptr<ARL::Reflection::ValueTable>& values)
{
	if (userPluginPath().isEmpty())
	{
		ARL::StandardOut::singleton()->print(ARL::MESSAGE_WARNING,
	...
	if (!pluginMetadataString.isEmpty())
	{
		...
		if (!ARL::WebParser::parseJSONTable(pluginMetadataString.toUtf8().constData(), readValues) )
		{
			ARL::StandardOut::singleton()->print(ARL::MESSAGE_WARNING,
...
void RobloxPluginHost::writePluginMetadata(const shared_ptr<ARL::Reflection::ValueTable>& values)
{
	std::stringstream outputBuffer;
	bool needComma = false;
	outputBuffer << "{";
	ARL::Stats::JsonWriter writer(outputBuffer, needComma);
...
void RobloxPluginHost::readPluginSettings(int assetId,
	boost::shared_ptr<ARL::Reflection::ValueTable>& settings)
{
	QDir pluginFolder = getSpecificPluginFolder(assetId);
	...
	if (!settingsFileContents.isEmpty())
	{
		...
		if (  ARL::WebParser::parseJSONTable(settingsFileContents.toStdString(), readValues) )
		{
			*settings = *readValues;
		}
		else
...
void RobloxPluginHost::writePluginSettings(int assetId,
	const boost::shared_ptr<ARL::Reflection::ValueTable>& settings)
{
	std::stringstream stream;
    bool needComma = false;
	stream << "{";
    ARL::Stats::JsonWriter writer(stream, needComma);
...
void RobloxPluginHost::openScriptDoc(shared_ptr<ARL::Instance> instance, int lineNumber)
{
	if (ARL::Instance::fastSharedDynamicCast<ARL::Script>(instance) ||
		ARL::Instance::fastSharedDynamicCast<ARL::ModuleScript>(instance))
		QMetaObject::invokeMethod(&RobloxDocManager::Instance(), "openDoc", Qt::QueuedConnection, Q_ARG(shared_ptr<ARL::Instance>, instance), Q_ARG(int, lineNumber));
}
...
void RobloxPluginHost::exportPlace(std::string filePath, ARL::ExporterSaveType exportType)
{
...
{
    throw ARL::runtime_error("%s %s", title, msg);
}

shared_ptr<ARL::Instance> RobloxPluginHost::csgUnion(shared_ptr<const ARL::Instances> instances, ARL::DataModel* dataModel)
{
    ...
    ARL::CSGOperations csgOps( dataModel, operationFailed );
    shared_ptr<ARL::PartOperation> partOperation;
    csgOps.doUnion( instances->begin(), instances->end(), partOperation );
    return partOperation;
}

shared_ptr<const ARL::Instances> RobloxPluginHost::csgNegate(shared_ptr<const ARL::Instances> instances, ARL::DataModel* dataModel)
{
    if (!FFlag::EnableCSGScripting)
        throw std::runtime_error("Scripted CSG operations are disabled");
...
shared_ptr<const ARL::Instances> RobloxPluginHost::csgSeparate(shared_ptr<const ARL::Instances> instances, ARL::DataModel* dataModel)
{
    if (!FFlag::EnableCSGScripting)
        throw std::runtime_error("Scripted CSG operations are disabled");
...
void RobloxPluginHost::handleDragEnterEvent(ARL::DataModel* dataModel, shared_ptr<const ARL::Instances> instances, ARL::Vector2 location)
{
	if (!m_pNotifier)
		return;
