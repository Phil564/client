...
{
public:
	StringPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	...
	void pushInstanceValue(ARL::Instance *pInstance) 
	{
	...
	void pullInstanceValue(ARL::Instance *pInstance) 
	{
		if (!pInstance || !m_pPropertyDescriptor)
			return;
...
class BoolPropertyItem:public PropertyItem
{
	int  m_isChecked;
public:
	BoolPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	, m_isChecked(0)
	{		
	...
	QString getTextValue(const ARL::Instance*) 
	{	return QString("");}
	...
	int compareInstanceValues(ARL::Instance *pPreviousInstance, ARL::Instance *pCurrentInstance)
	{	
			...
			if (!RobloxSettings().value(kLoadStringEnabledDialogDisabled, false).toBool())
			{
				QtUtilities::ARLConfirmationMessageBox msgBox("Activating 'LoadStringEnabled' property might make your game vulnerable to exploits \
													   (for more information\
													    <a href=\"http://wiki.lambda.cam/index.php?title=Security#LoadStringEnabled\">click here</a>).\
...
DoublePropertyItem::DoublePropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	, m_SpinBox(NULL)
	, m_Slider(NULL)
{
...
void DoublePropertyItem::setVariantValue(const ARL::Reflection::Variant &value)
{
	if (FFlag::StudioPropertyErrorOutput)
	{
		...
		if (str1 != str2)
		{
			m_variantValue = ARL::Reflection::Variant(str1.toDouble());
		}
		else
		{
...
class EnumPropertyItem: public PropertyItem
{
	...
	static bool shouldDisplayEnumItem(const ARL::Reflection::EnumDescriptor::Item* item)
	{
		ARL::Reflection::Metadata::EnumItem* m = ARL::Reflection::Metadata::Reflection::singleton()->get(*item);

		if (m && !m->isBrowsable())
			return false;

		return !ARL::Reflection::Metadata::Item::isDeprecated(m, *item);
	}

public:
	EnumPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	, m_bUpdateInProgress(false)
	{
		const ARL::Reflection::EnumPropertyDescriptor* pEnumDesc = dynamic_cast<const ARL::Reflection::EnumPropertyDescriptor*>(pPropertyDescriptor);
		if (pEnumDesc)
		{
			std::vector<const ARL::Reflection::EnumDescriptor::Item* >::const_iterator curIter, endIter;
	...
	void pushInstanceValue(ARL::Instance *pInstance)
	{
		const ARL::Reflection::EnumPropertyDescriptor* pEnumDesc = dynamic_cast<const ARL::Reflection::EnumPropertyDescriptor*>(m_pPropertyDescriptor);
		if (pEnumDesc)
	...
	void onEvent(const QObject *, const QString &arg) 
	{
		...
		ARL::ScopedAssign<bool> ignoreCommitModif(m_bUpdateInProgress, true);
...
BrickColorPropertyItem::BrickColorPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
: PropertyItem(pPropertyDescriptor)
...
{
	ARL::BrickColor color = m_variantValue.get<ARL::BrickColor>();
...
void BrickColorPropertyItem::updateIcon()
{
	...
	{
		ARL::BrickColor rbxColor = m_variantValue.get<ARL::BrickColor>();
...
void BrickColorPropertyItem::buttonClicked(const QString& buttonName)
{
	...
	{
		setVariantValue(ARL::BrickColor(selectedColor));
		commitModification();
...
void BrickColorPropertyItem::updatePropertyValueQColor(QColor selectedColor)
{
	...
	ARL::BrickColor brickColor = ARL::BrickColor::closest(G3D::Color3::fromARGB(selectedColor.rgba()));
...
void BrickColorPropertyItem::updatePropertyValue(int selectedColor)
{
	...
	ARL::BrickColor brickColor(selectedColor);
...
// ColorPropertyItem
//--------------------------------------------------------------------------------------------
ColorPropertyItem::ColorPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	, m_pPopupLaunchEditor(NULL)
...
void ColorPropertyItem::setModelDataSafe(QWidget *editor)
{
	if (m_pPopupLaunchEditor && m_pProxyLineEdit && (m_pProxyLineEdit->text() != text(1)))
	{
		...
		if (ARL::StringConverter<G3D::Color3>::convertToValue(m_pProxyLineEdit->text().toStdString(), editedColor))
		{
...
public:
	...
	FontPropertyItem(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
	    : PropertyItem(pPropertyDescriptor)
	{
	}
...
public:
	...
	DirPropertyItem(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
	    : PropertyItem(pPropertyDescriptor)
	{
	}
...
class RefPropertyItem: public PropertyItem
{
	QString    m_TextValue;
public:
	RefPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	{
		//read only
	...
	void pushInstanceValue(ARL::Instance *pInstance)
	{
		const ARL::Reflection::RefPropertyDescriptor* pRefProp = dynamic_cast<const ARL::Reflection::RefPropertyDescriptor*>(m_pPropertyDescriptor);
		const ARL::Instance *pParentInstance = boost::polymorphic_downcast<const ARL::Instance*>(pRefProp->getRefValue(pInstance));
		setVariantValue(pParentInstance ? pParentInstance->getName() : std::string(""));
	}	

	void pullInstanceValue(ARL::Instance*)
	{
		//no value change!!!
	}
};
...
{
	QString                               m_TextValue;
	boost::shared_ptr<ARL::ModelInstance> m_pModelInstance;
	QPointer<PrimaryPartSelectionEditor>  m_pPrimaryPartSelEditor;
	
public:
	PrimaryPartPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	...
	void pushInstanceValue(ARL::Instance *pInstance)
	{
		...
		ARL::Selection* selection = ARL::ServiceProvider::find<ARL::Selection>(pInstance);
		...
		m_pModelInstance = shared_from(ARL::Instance::fastDynamicCast<ARL::ModelInstance>(pInstance));
		if (!m_pModelInstance)
		...
		std::string strVal;
		ARL::PartInstance* pPartInstance = m_pModelInstance->getPrimaryPartSetByUser();
		if (pPartInstance)
	...
	void pullInstanceValue(ARL::Instance* pInstance)
	{
		...
		std::string strVal;
		ARL::ModelInstance* mInstance = ARL::Instance::fastDynamicCast<ARL::ModelInstance>(pInstance);
		if (mInstance)
		{
			ARL::PartInstance* pPartInstance = mInstance->getPrimaryPartSetByUser();
			if (pPartInstance)
				strVal = pPartInstance->getName().c_str();
		}
...
public:
	PhysicalPropertiesItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	...
	{	
		int result = 0;
		ARL::Reflection::TypedPropertyDescriptor<ARL::PhysicalProperties>* pTypedDesc = (ARL::Reflection::TypedPropertyDescriptor<ARL::PhysicalProperties>*)(m_pPropertyDescriptor);
		if (pTypedDesc)
		{
			ARL::PhysicalProperties value1 = pTypedDesc->getValue(pPreviousInstance);
			ARL::PhysicalProperties value2 = pTypedDesc->getValue(pCurrentInstance);
	...
	{
		ARL::PhysicalProperties modifiedProperties;
		if (m_isChecked == 1)
		{
			modifiedProperties = ARL::PhysicalProperties( 
	...
	void pullInstanceValue(ARL::Instance *pInstance)
	{
		...
		ARL::PhysicalProperties value = pTypedDesc->getValue(pInstance);
		...
		if (m_checkToggled)
		{
			...
			{
				ARL::PartInstance* ourPart = ARL::Instance::fastDynamicCast<ARL::PartInstance>(pInstance);
				ARL::PhysicalProperties materialProp = ARL::MaterialProperties::generatePhysicalMaterialFromPartMaterial(ourPart->getRenderMaterial());
				value = materialProp;
			}
			else if (!m_isChecked && value.getCustomEnabled())
			{
				value = ARL::PhysicalProperties();
			}
		}
		else
		{
		...
		{
			value = ARL::PhysicalProperties(m_pChild[PhysProp_Density]->getVariantValue().get<double>(), currentFriction, currentElasticity, currentFrictionWeight, currentElasticityWeight);
		}
		else if (m_componentToUpdate == PhysProp_Friction)
		{
			value = ARL::PhysicalProperties(currentDensity, m_pChild[PhysProp_Friction]->getVariantValue().get<double>(), currentElasticity, currentFrictionWeight, currentElasticityWeight);
		}
		else if (m_componentToUpdate == PhysProp_Elasticity)
		{
			value = ARL::PhysicalProperties(currentDensity, currentFriction, m_pChild[PhysProp_Elasticity]->getVariantValue().get<double>(), currentFrictionWeight, currentElasticityWeight);
		}
		else if (m_componentToUpdate == PhysProp_FrictionWeight)
		{
			value = ARL::PhysicalProperties(currentDensity, currentFriction, currentElasticity, m_pChild[PhysProp_FrictionWeight]->getVariantValue().get<double>(), currentElasticityWeight);
		}
		else if (m_componentToUpdate == PhysProp_ElasticityWeight)
		{
			value = ARL::PhysicalProperties(currentDensity, currentFriction, currentElasticity, currentFrictionWeight, m_pChild[PhysProp_ElasticityWeight]->getVariantValue().get<double>());
		}*/

		m_variantValue = ARL::Reflection::Variant(value);
	...
	void updateChildren()
	{
		...
		ARL::PhysicalProperties propertyVal = m_variantValue.get<ARL::PhysicalProperties>();
	...
	virtual void setVariantValue(const ARL::Reflection::Variant &value)
	{
		...
		{
			updateChildren();
			ARL::PhysicalProperties currentProperty = m_variantValue.get<ARL::PhysicalProperties>();
			ARL::PhysicalProperties vectorVal;
			if (currentProperty.getCustomEnabled())
			{
				vectorVal = ARL::PhysicalProperties(
...
class VectorPropertyItem: public PropertyItem
{
	...
	int                 m_componentToUpdate;
public:
	VectorPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	, m_componentToUpdate(-1)
	{
	...
	{	
		...
		ARL::Reflection::TypedPropertyDescriptor<G3D::Vector3>* pTypedDesc = (ARL::Reflection::TypedPropertyDescriptor<G3D::Vector3>*)(m_pPropertyDescriptor);
		if (pTypedDesc)
		{			
	...
	void setModelData(QWidget *editor)
	{
		...
		if (pLineEdit->text() != text(1))
		{
			...
			if (ARL::StringConverter<G3D::Vector3>::convertToValue(pLineEdit->text().toStdString(), vectorVal))
			{
...
class RectPropertyItem: public PropertyItem
{
	...
	int                 m_componentToUpdate;
public:
	RectPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	, m_componentToUpdate(-1)
	{
	...
	int compareInstanceValues(ARL::Instance *pPreviousInstance, ARL::Instance *pCurrentInstance)
	{
		...
		ARL::Reflection::TypedPropertyDescriptor<G3D::Rect2D>* pTypedDesc = (ARL::Reflection::TypedPropertyDescriptor<G3D::Rect2D>*)(m_pPropertyDescriptor);
		if (pTypedDesc)
		{
	...
	void pullInstanceValue(ARL::Instance *pInstance)
	{
        ...
        //*pTypedDesc = *((ARL::Reflection::TypedPropertyDescriptor<G3D::Rect2D>*) new G3D::Rect2D());
	...
	void setModelData(QWidget *editor)
	{
		...
		{
			G3D::Rect2D rectVal;
			if (ARL::StringConverter<G3D::Rect2D>::convertToValue(pLineEdit->text().toStdString(), rectVal))
			{
...
public:
	...
	int compareInstanceValues(const ARL::UDim &previousVal, const ARL::UDim &currentVal)
	{
		int result = 1;
	...
	QString getTextValue()
	{
		ARL::UDim uDim = m_variantValue.get<ARL::UDim>();
	...
	void pullInstanceValue(ARL::Instance *pInstance)
	{
		ARL::Reflection::TypedPropertyDescriptor<ARL::UDim2>* pTypedDesc = (ARL::Reflection::TypedPropertyDescriptor<ARL::UDim2>*)(m_pParentItem->propertyDescriptor());
		if (!pTypedDesc)
			return;

		//set only the modified component
		ARL::UDim2 value = pTypedDesc->getValue(pInstance);

		//nothing special required in case there's no component modified
		if (m_componentToUpdate < 0)
		{				
			value[m_pComponentID] = m_variantValue.get<ARL::UDim>();
		}
		else
		{
			if (m_componentToUpdate > 0)
	...
	void setModelData(QWidget *editor)
	{
		...
		{
			ARL::UDim udimVal;
			if (ARL::StringConverter<ARL::UDim>::convertToValue(pLineEdit->text().toStdString(), udimVal))
			{
	...
	{
		//set appropriate values in children
		ARL::UDim udimVal = m_variantValue.get<ARL::UDim>();		
...
{	
	...
	int                  m_componentToUpdate;
public:
	UDim2PropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	, m_componentToUpdate(-1)
	{
	...
	{	
		...
		ARL::Reflection::TypedPropertyDescriptor<ARL::UDim2>* pTypedDesc = (ARL::Reflection::TypedPropertyDescriptor<ARL::UDim2>*)(m_pPropertyDescriptor);
		if (pTypedDesc)
		{			
			ARL::UDim2 value1 = pTypedDesc->getValue(pPreviousInstance);
			ARL::UDim2 value2 = pTypedDesc->getValue(pCurrentInstance);
			result = value1 == value2;
	...
	{			
		//get modified values
		ARL::UDim2 modifiedValue;
		for (int ii=0; ii<2; ++ii)
			modifiedValue[ii] = m_pChild[ii]->getVariantValue().get<ARL::UDim>();
	...
	void pullInstanceValue(ARL::Instance *pInstance)
	{
		...
		//set only the modified component
		ARL::UDim2 value = pTypedDesc->getValue(pInstance);
		value[m_componentToUpdate] = m_pChild[m_componentToUpdate]->getVariantValue().get<ARL::UDim>();
		pTypedDesc->setValue(pInstance, value);		
	}

	QString getTextValue()
	{		
		ARL::UDim2 uDim2 = m_variantValue.get<ARL::UDim2>();
	...
	void setModelData(QWidget *editor)
	{
		...
		{
			ARL::UDim2 udimVal;
			if (ARL::StringConverter<ARL::UDim2>::convertToValue(pLineEdit->text().toStdString(), udimVal))
			{
				setVariantValue(udimVal);
				commitModification();
	...
	{
		//set appropriate values in children
		ARL::UDim2 udimVal = m_variantValue.get<ARL::UDim2>();
		for (int ii=0; ii<2; ++ii)
...
{
	ARL::NormalId    m_componentId;
public:
	FaceComponentItem(PropertyItem *pParentItem, const QString &name, ARL::NormalId component)
	: PropertyItem(pParentItem, name)
	, m_componentId(component)
	{
	}

	int compareInstanceValues(const ARL::Faces &previousFaces, const ARL::Faces &currentFaces)
	{	return previousFaces.getNormalId(m_componentId) == currentFaces.getNormalId(m_componentId); }	

	QString getTextValue()
	{
		ARL::Faces faceVal = m_variantValue.get<ARL::Faces>();		
		return ARL::StringConverter<bool>::convertToString(faceVal.getNormalId(m_componentId)).c_str();
	}

	void getValue(ARL::Faces& value)
	{		
		ARL::Faces faceVal = m_variantValue.get<ARL::Faces>();			
...
class FacesPropertyItem: public PropertyItem
{
	...
	int                 m_componentToUpdate;
public:
	FacesPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	, m_componentToUpdate(-1)
	{
		//create children
		m_pChild[0] = new FaceComponentItem(this, "Top",    ARL::NORM_Y);
		m_pChild[1] = new FaceComponentItem(this, "Bottom", ARL::NORM_Y_NEG);
		m_pChild[2] = new FaceComponentItem(this, "Back",   ARL::NORM_Z);
		m_pChild[3] = new FaceComponentItem(this, "Front",  ARL::NORM_Z_NEG);
		m_pChild[4] = new FaceComponentItem(this, "Right",  ARL::NORM_X);
		m_pChild[5] = new FaceComponentItem(this, "Left",   ARL::NORM_X_NEG);
	...
	{	
		...
		const ARL::Reflection::TypedPropertyDescriptor<ARL::Faces>* pFacesPropDesc = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<ARL::Faces>*>(m_pPropertyDescriptor);
		if (pFacesPropDesc)
		{			
			ARL::Faces value1 = pFacesPropDesc->getValue(pPreviousInstance);
			ARL::Faces value2 = pFacesPropDesc->getValue(pCurrentInstance);
			result = value1 == value2;
	...
	void pullInstanceValue(ARL::Instance *pInstance)
	{
		...
		ARL::Reflection::TypedPropertyDescriptor<ARL::Faces>* pTypedDesc = (ARL::Reflection::TypedPropertyDescriptor<ARL::Faces>*)(m_pPropertyDescriptor);
		if (!pTypedDesc)
			return;

		//set only the modified component
		ARL::Faces value = pTypedDesc->getValue(pInstance);
	...
	QString getTextValue()
	{		
		std::string result = ARL::StringConverter<ARL::Faces>::convertToString(m_variantValue.get<ARL::Faces>());
	...
	void setModelData(QWidget *editor)
	{
		...
		{
			ARL::Faces faceVal;
			if (ARL::StringConverter<ARL::Faces>::convertToValue(pLineEdit->text().toStdString(), faceVal))
			{
...
{
public:
	ContentPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
...
{
public:
	ImageAssetPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
...
public:
	ScriptAssetPropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
	: PropertyItem(pPropertyDescriptor)
	...
	QWidget* createEditor(QWidget *parent, const QStyleOptionViewItem &option)
	{
		...
		{
			ARL::ContentId contentId = ARL::ContentId::fromGameAssetName(scriptNames[i].toStdString());
	...
	void setModelData(QWidget *editor)
	{
		...
		if (comboBox->currentText() != text(1))
		{
			...
			ARL::ScopedAssign<bool> ignoreCommitModif(currentlyCommitting, true);
			setVariantValue(ARL::ContentId(currentText.toStdString()));
			commitModification();
		}
	}
};
...
{
public:
    NumberSequencePropertyItem(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
        : PropertyItem(pPropertyDescriptor)
    {
    }
    ...
    QString getTextValue()
    {
        ARL::NumberSequence val = m_variantValue.get<ARL::NumberSequence>();
        const std::vector< ARL::NumberSequenceKeypoint >& keys = val.getPoints();
    ...
    void setModelData(QWidget *editor)
    {
        ...
        setVariantValue( ARL::NumberSequence(constval) );
        commitModification();
    }
    ...
    void onCurveClicked()
    {
        ...
        ARL::Instance* inst = *selected.begin();
...
class NumberRangePropertyItem : public PropertyItem
{
    ...
    int m_componentToUpdate;
public:
    NumberRangePropertyItem(const ARL::Reflection::PropertyDescriptor* pd) : PropertyItem(pd)
    {
    ...
    int compareInstanceValues(ARL::Instance* prev, ARL::Instance* cur)
    {
        int result = 0;

        if (const ARL::Reflection::TypedPropertyDescriptor<ARL::NumberRange>* propd = static_cast<const ARL::Reflection::TypedPropertyDescriptor<ARL::NumberRange>* > ( m_pPropertyDescriptor ))
        {
             ARL::NumberRange a = propd->getValue(prev);
             ARL::NumberRange b = propd->getValue(cur);
             if (a.min != b.min)
             {
                 m_pMin->setText(1, "");
                 result = -1;
    ...
    void setVariantValue(const ARL::Reflection::Variant& val)
    {
    ...
    QString getTextValue()
    {
        ARL::NumberRange r = m_variantValue.get<ARL::NumberRange>();
        if(r.min == r.max)
        {
            return format(r.min);
        }
    ...
    {			
        ...
        ARL::NumberRange modifiedValue(std::min(a,b), std::max(a,b));
    ...
    {
        //set appropriate values in children
        ARL::NumberRange val = m_variantValue.get<ARL::NumberRange>();
    ...
    void setModelData(QWidget *editor)
    {
        ...
        if (pLineEdit->text() != text(1))
        {
            ...
            setVariantValue(ARL::NumberRange(min, max));
            commitModification();
...
public:
    ...
    ColorSequencePropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor) : PropertyItem(pPropertyDescriptor)
    {
    ...
    {
        m_variantValue = value;
        ARL::ColorSequence cs = value.get<ARL::ColorSequence>();
        ...
        ARL::Color3 a = m_start->getVariantValue().get<ARL::Color3>();
        ARL::Color3 b = m_end->getVariantValue().get<ARL::Color3>();
    ...
    void syncChildren(PropertyItem *pChildItem)
    {
        ...
        setVariantValue( ARL::ColorSequence(a,b) );
        commitModification(true);
    }

    void updateIcon()
    {
        ARL::ColorSequence cs = getVariantValue().get<ARL::ColorSequence>();
        ARL::Color3 a = cs.start().value;
        ARL::Color3 b = cs.end().value;
        ...
        {
            ARL::Color3 c = a.lerp(b, x/float(W-1) );
...
// PropertyItem
//--------------------------------------------------------------------------------------------
PropertyItem::PropertyItem(const ARL::Reflection::PropertyDescriptor *pPropertyDescriptor)
...
void PropertyItem::init(const QString &itemName)
{
	...
	if (m_pPropertyDescriptor) 
	{
        ...
        ARL::Reflection::Metadata::Item* metadata = ARL::Reflection::Metadata::Reflection::singleton()->get(*m_pPropertyDescriptor);
...
PropertyItem* PropertyItem::createItem(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
	const ARL::Reflection::TypedPropertyDescriptor<bool>* pBoolProp = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<bool>*>(pPropertyDescriptor);
	if (pBoolProp)
		return new BoolPropertyItem(pPropertyDescriptor);

	const ARL::Reflection::TypedPropertyDescriptor<int>* pIntProp = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<int>*>(pPropertyDescriptor);
	if (pIntProp)
		return new IntPropertyItem(pPropertyDescriptor);
			
	const ARL::Reflection::TypedPropertyDescriptor<float>* pFloatProp = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<float>*>(pPropertyDescriptor);
	if (pFloatProp)
		return new DoublePropertyItem(pPropertyDescriptor);

	if (const ARL::Reflection::TypedPropertyDescriptor<ARL::BrickColor>* pBrickColorProp = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<ARL::BrickColor>*>(pPropertyDescriptor))
	if (pBrickColorProp)
	...
	const ARL::Reflection::TypedPropertyDescriptor<G3D::Vector3>* pVector3Prop = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<G3D::Vector3>*>(pPropertyDescriptor);
	if (pVector3Prop)
		return new VectorPropertyItem(pPropertyDescriptor);	

	const ARL::Reflection::TypedPropertyDescriptor<ARL::PhysicalProperties>* pPhysicalPropertiesProp = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<ARL::PhysicalProperties>*>(pPropertyDescriptor);
	if (pPhysicalPropertiesProp)
		return new PhysicalPropertiesItem(pPropertyDescriptor);

	const ARL::Reflection::RefPropertyDescriptor* pRefProp = dynamic_cast<const ARL::Reflection::RefPropertyDescriptor*>(pPropertyDescriptor);
	if (pRefProp)
	{
	...
	const ARL::Reflection::TypedPropertyDescriptor<ARL::Faces>* pFacesProp = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<ARL::Faces>*>(pPropertyDescriptor);
	if (pFacesProp)
		return new FacesPropertyItem(pPropertyDescriptor);	

	const ARL::Reflection::TypedPropertyDescriptor<ARL::UDim2>* pUdim2Prop = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<ARL::UDim2>*>(pPropertyDescriptor);
	if (pUdim2Prop)	
		return new UDim2PropertyItem(pPropertyDescriptor);

	const ARL::Reflection::TypedPropertyDescriptor<ARL::MeshId>* pMeshProp = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<ARL::MeshId>*>(pPropertyDescriptor);
	if (pMeshProp)
	...
	{
		const ARL::Reflection::TypedPropertyDescriptor<ARL::AnimationId>* pAnimationProp = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<ARL::AnimationId>*>(pPropertyDescriptor);
		if (pAnimationProp)		
			return new ContentPropertyItem(pPropertyDescriptor);
	}
	
	const ARL::Reflection::TypedPropertyDescriptor<QFont>* pFontProp = dynamic_cast<const ARL::Reflection::TypedPropertyDescriptor<QFont>*>(pPropertyDescriptor);
	if ( pFontProp )
    ...
    const ARL::Reflection::TypedPropertyDescriptor<ARL::NumberRange>* pNRProp = dynamic_cast< const ARL::Reflection::TypedPropertyDescriptor<ARL::NumberRange>*>(pPropertyDescriptor);
    if( pNRProp )
        return new NumberRangePropertyItem(pPropertyDescriptor);

    const ARL::Reflection::TypedPropertyDescriptor<ARL::ColorSequence>* pCSProp = dynamic_cast< const ARL::Reflection::TypedPropertyDescriptor<ARL::ColorSequence>*>(pPropertyDescriptor);
    if( pCSProp )
...
bool materialPropertiesIsEnabled(const InstanceList &instances)
{
	...
	{
		if (ARL::PartInstance* part = (*curIter)->fastDynamicCast<ARL::PartInstance>())
		{
			if (ARL::Workspace* currentWorkspace = ARL::Workspace::findWorkspace(part))
			{
...
bool PropertyItem::currentlyHidden(const InstanceList &instances)
{
	...
	if (!FFlag::CSGPhysicsLevelOfDetailEnabled && (*propertyDescriptor() == ARL::PartOperation::prop_CollisionFidelity))
	{
	...
	if (!DFFlag::MaterialPropertiesEnabled && (*propertyDescriptor() == ARL::Workspace::prop_physicalPropertiesMode))
	{
	...
	{
		if (*propertyDescriptor() == ARL::PartInstance::prop_Friction ||
			*propertyDescriptor() == ARL::PartInstance::prop_Elasticity)
		{
	...
	{
		if (*propertyDescriptor() == ARL::PartInstance::prop_CustomPhysicalProperties)
		{
...
int PropertyItem::compareInstanceValues(ARL::Instance *pPreviousInstance, ARL::Instance *pCurrentInstance)
{
	if (!m_pPropertyDescriptor)
		return 0;
...
void PropertyItem::pushInstanceValue(ARL::Instance *pInstance) 
{ 
	if (!pInstance || !m_pPropertyDescriptor)
		return;

	//set variant value
	ARL::Reflection::Variant value;
	m_pPropertyDescriptor->getVariant(pInstance, value);	
	setVariantValue(value);
}

void PropertyItem::pullInstanceValue(ARL::Instance *pInstance)
{
...
void PropertyItem::setVariantValue(const ARL::Reflection::Variant &value)
{
	m_variantValue = value;
	//also set its text value
	setTextValue(getTextValue());
...
ARL::Reflection::Variant PropertyItem::getVariantValue()
{ return m_variantValue; }

void PropertyItem::addClassDescriptor(const ARL::Reflection::ClassDescriptor* pClassDescriptor)
{
	if (!m_pPropertyDescriptor)
		return;
...
{	
	try 
	{
		...
		{		
			...
			boost::shared_ptr<ARL::DataModel> pDataModel = getTreeWidget()->getDataModel();
			if ( pDataModel.get() )
			{
				...
				//for undo/redo
				if (requestWaypoint)
					ARL::ChangeHistoryService::requestWaypoint(m_pPropertyDescriptor->name.c_str(), pDataModel.get());
...
bool PropertyItem::handleCollisionLocally(ARL::Instance* pInstance)
{
    if (!ARL::AdvArrowTool::advCollisionCheckMode && 
        m_variantValue.isType<ARL::Vector3>())
    {
        if (ARL::PartInstance* partInstance = pInstance->fastDynamicCast<ARL::PartInstance>())
        {
            QString propertyName = m_pPropertyDescriptor->name.c_str();
            if (propertyName == "Position")
            {
                ARL::Vector3 position = m_variantValue.get<ARL::Vector3>();
            ...
            {
                ARL::Vector3 rotationVector = m_variantValue.get<ARL::Vector3>();
                G3D::Matrix3 rotation = G3D::Matrix3::fromEulerAnglesXYZ(ARL::Math::degreesToRadians(rotationVector.x),ARL::Math::degreesToRadians(rotationVector.y),ARL::Math::degreesToRadians(rotationVector.z));
