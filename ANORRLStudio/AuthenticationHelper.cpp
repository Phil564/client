...
QString AuthenticationHelper::doHttpGetRequest(const QString& str, const ARL::HttpAux::AdditionalHeaders& externalHeaders)
{
	std::string result = "";
	try 
	{
...
{
	return ARL::MachineIdUploader::uploadMachineId(qPrintable(RobloxSettings::getBaseURL()))
		!= ARL::MachineIdUploader::RESULT_MachineBanned;
}
...
bool AuthenticationHelper::eventFilter(QObject * watched, QEvent * evt)
{
	...
	else if (eventType == QEvent::ApplicationActivate)
	{
		...
		if (QString::compare(savedCookieKey, currentCookieKey, Qt::CaseSensitive) != 0)	
		{
			...
			ARLASSERT(getHttpUserId() == getWebKitUserId());
		}
	}
...
bool AuthenticationHelper::authenticateHttpSession()
{
	...
	{
		ARL::StandardOut::singleton()->print(ARL::MESSAGE_ERROR, "Unable to authenticate user. Request timed out.");
	}
...
QString AuthenticationHelper::getHttpRequest(const QString& urlString, const ARL::HttpAux::AdditionalHeaders& externalHeaders)
{
	static QMutex httpRequestMutex;
	QMutexLocker lock(&httpRequestMutex);

	ARL::HttpOptions options;
	options.addHeader("ARLAuthenticationNegotiation:", ::GetBaseURL());

	for (ARL::HttpAux::AdditionalHeaders::const_iterator iter = externalHeaders.begin(); iter != externalHeaders.end(); ++iter)
		options.addHeader(iter->first, iter->second);
	
	ARL::HttpFuture request = ARL::HttpAsync::get(urlString.toStdString(), options);
	std::string result;
	...
	{
		ARL::StandardOut::singleton()->printf(ARL::MESSAGE_ERROR, "Error while authenticating user: %s", e.what());
	}
...
void AuthenticationHelper::httpSessionAuthenticationDone()
{
	...
	ARLASSERT(m_CurrentHttpRequestState != -1);
