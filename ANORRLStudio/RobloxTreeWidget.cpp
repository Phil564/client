...
bool DepthCompare::operator() (const RobloxTreeWidgetItem* lhs, const RobloxTreeWidgetItem* rhs)
{
...
bool InstanceUpdateHandler::onChildAdded(shared_ptr<ARL::Instance> pChild)
{
	...
	if (RobloxIDEDoc::getIsCloudEditSession() && ARL::Security::Context::current().identity == ARL::Security::Replicator_)
...
RobloxTreeWidgetItem* InstanceUpdateHandler::processChildAdd(shared_ptr<ARL::Instance> pInstance)
{
	RobloxTreeWidget *pTreeWidget = getTreeWidget();
...
RobloxTreeWidgetItem::RobloxTreeWidgetItem(int index, RobloxTreeWidget *pTreeWidget, boost::shared_ptr<ARL::Instance> pInstance)
...
RobloxTreeWidgetItem::RobloxTreeWidgetItem(int index, RobloxTreeWidgetItem *pParentWidgetItem, boost::shared_ptr<ARL::Instance> pInstance)
: InstanceUpdateHandler(pInstance)
, m_ItemInfo(0)
, m_queuedForDeletion(false)
{
...
bool RobloxTreeWidgetItem::onChildAdded(shared_ptr<ARL::Instance> pChild)
{
...
bool RobloxTreeWidgetItem::onChildRemoved(shared_ptr<ARL::Instance> pChild)
{		
	if (FFlag::StudioTreeWidgetCheckDeletingFlagWhenDoingUpdates && m_queuedForDeletion) return false;
...
int RobloxTreeWidgetItem::getIndexToInsertAt(shared_ptr<ARL::Instance> pInstance)
{
	int numChild = childCount();
	if (!numChild)
		return -1;
...
bool RobloxTreeWidgetItem::isExplorerItem(const shared_ptr<ARL::Instance>& pInstance)
{
...
int RobloxTreeWidgetItem::getImageIndex(const shared_ptr<ARL::Instance>& pInstance)
{
	boost::shared_ptr<const ARL::Reflection::Metadata::Reflection> pReflection = ARL::Reflection::Metadata::Reflection::singleton();
	ARL::Reflection::Metadata::Class* pClassData = pReflection ? pReflection->get(pInstance->getDescriptor(), true) : NULL;
	if(pClassData)
...
int RobloxTreeWidgetItem::getItemType(const shared_ptr<ARL::Instance>& pInstance)
{
	boost::shared_ptr<const ARL::Reflection::Metadata::Reflection> pReflection = ARL::Reflection::Metadata::Reflection::singleton();
...
RobloxTreeWidget::RobloxTreeWidget(boost::shared_ptr<ARL::DataModel> pDataModel)
...
void RobloxTreeWidget::addInstance(const ARL::Instance *pInstance, RobloxTreeWidgetItem *pTreeWidgetItem) 
{
	m_InstanceMap[pInstance] = pTreeWidgetItem;
}

void RobloxTreeWidget::initTreeView(boost::shared_ptr<ARL::DataModel> pDataModel)
{
...
void RobloxTreeWidget::createTreeRoot(const boost::shared_ptr<ARL::Instance> pInstance)
{
...
ARL::Selection* RobloxTreeWidget::getSelection()
{	return ARL::ServiceProvider::create<ARL::Selection>(m_pDataModel.get()); }

boost::shared_ptr<ARL::DataModel> RobloxTreeWidget::dataModel()
{	return m_pDataModel; }

void RobloxTreeWidget::eraseInstance(ARL::Instance *pInstance)
{ 
...
RobloxTreeWidgetItem* RobloxTreeWidget::findItemFromInstance(const ARL::Instance *pInstance)
{
	InstanceMap::iterator iter = m_InstanceMap.find(pInstance);
	if (iter != m_InstanceMap.end())	
		return iter->second;
...
void RobloxTreeWidget::scrollToInstance(ARL::Instance *pInstance)
{
	if(!pInstance)
		return;
...
int RobloxTreeWidget::getIndexToInsertAt(shared_ptr<ARL::Instance> pInstance)
{
...
bool RobloxTreeWidget::onChildAdded(shared_ptr<ARL::Instance> pChild)
{
...
bool RobloxTreeWidget::onChildRemoved(shared_ptr<ARL::Instance> pChild)
{
	if (FFlag::StudioTreeWidgetCheckDeletingFlagWhenDoingUpdates && m_bDeletionRequested) return false;
...
void RobloxTreeWidget::onDescendantAdded(shared_ptr<ARL::Instance> pDescendant)
{
	if (!isFilterEmpty())
	{
...
void RobloxTreeWidget::onInstanceSelectionChanged(const ARL::SelectionChanged& evt)
{
...
bool RobloxTreeWidget::isSearchTimeUp(const ARL::Time& startTime, int duration)
{
...
bool RobloxTreeWidget::filterAncestors(shared_ptr<ARL::Instance> instance)
{
...
bool RobloxTreeWidget::filterInstance(shared_ptr<ARL::Instance> instance)
{
...
bool RobloxTreeWidget::filterWidgetRecursive(shared_ptr<ARL::Instance> instance, FilterDeque_SPTR& previousFilterStack, const ARL::Time& startTime)
{
	if (!RobloxTreeWidgetItem::isExplorerItem(instance))
		return false;
...
bool RobloxTreeWidget::isFoundItem(shared_ptr<ARL::Instance> instance)
{
	return m_foundItems.count(instance) > 0;
}

bool RobloxTreeWidget::isAncestorOfFoundItem(shared_ptr<ARL::Instance> instance)
{
	return m_ancestorsOfFoundItems.count(instance) > 0;
}

bool RobloxTreeWidget::isValidSelection(ARL::Instance *pInstance)
{
	if (!pInstance)
		return false;
...
void RobloxTreeWidget::updateSelectionState(ARL::Instance *pInstance)
{
	ARL::Selection* pSelection(getSelection());
