...
public:
	...
	{
		ARLPROFILER_SCOPE("Qt", "$Event");
		ARLPROFILER_LABELF("Qt", "%s::%d", receiver->objectName().toStdString().c_str(), event->type());
...
#endif
		// Following is required so that when we launch from QProcess new instance of ANORRLStudio should come to the top of Z-Order
