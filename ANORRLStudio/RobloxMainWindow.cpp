...
, m_cookieConstraintCheckDone(NULL)
{
	try
	{
		...
		setupLogging();
        ARL::Log::current()->writeEntry(ARL::Log::Information,"RobloxMainWindow::RobloxMainWindow - start");
		...
		ARL::Analytics::setAppVersion(RobloxSettings::getVersionString().toStdString());
    ...
    ARL::Log::current()->writeEntry(ARL::Log::Information,"RobloxMainWindow::RobloxMainWindow - end");
}

RobloxMainWindow::~RobloxMainWindow()
{
    ARL::Log::current()->writeEntry(ARL::Log::Information,"RobloxMainWindow::~RobloxMainWindow");
...
bool RobloxMainWindow::requestDocClose(IRobloxDoc& doc, bool closeIfLastDoc)
{
	...
	if ( doc.isModified() )
	{
		IRobloxDoc::ARLCloseRequest closeMode = doc.requestClose();
...
{
	ARLASSERT(RobloxDocManager::Instance().getCurrentDoc());
	ARLASSERT(RobloxDocManager::Instance().getCurrentDoc() == RobloxDocManager::Instance().getPlayDoc());
...
{
    ARL::Log::current()->writeEntry(ARL::Log::Information,"RobloxMainWindow::closeEvent");
...
void RobloxMainWindow::checkInternetConnectionSendCounter(ARL::Http& robloxRequest, ARL::Http& externalRequest)
{
...
bool RobloxMainWindow::handleFileOpen(const QString &fileName, IRobloxDoc::ARLDocType type, const QString &script)
{
    ARL::Log::current()->writeEntry(ARL::Log::Information, ARL::format("RobloxMainWindow::handleFileOpen %s", script.toStdString().c_str()).c_str());
    ...
    if (!newDoc->open(this, fileToOpen))
    {
        ...
        if (ARL::StandardOut::singleton() && (!fileToOpen.isEmpty()) && type == IRobloxDoc::IDE)
...
void RobloxMainWindow::updateWindowTitle()
{
    ...
    else
    {
		...
		case IRobloxDoc::ARLDocType::BROWSER:
			...
			break;
		case IRobloxDoc::ARLDocType::IDE:
			docTypeString = "Developing";
			break;
		case IRobloxDoc::ARLDocType::SCRIPT:
			docTypeString = "Scripting";
			break;
		case IRobloxDoc::ARLDocType::OBJECTBROWSER:
			docTypeString = "Browsing";
			break;
		}
...
void RobloxMainWindow::onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor)
{
