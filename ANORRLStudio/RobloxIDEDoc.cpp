...
void RobloxIDEDoc::InitializeDefaultsFromSettings()
{
	...
	ARL::Camera::CameraKeyMoveFactor = AuthoringSettings::singleton().getCameraKeyMoveSpeed();
	ARL::Camera::CameraShiftKeyMoveFactor = AuthoringSettings::singleton().getCameraShiftMoveSpeed();
	ARL::Camera::CameraMouseWheelMoveFactor = AuthoringSettings::singleton().getCameraMouseWheelMoveSpeed();

	// Turn on or off the dragger grid
	ARL::ArrowToolBase::showDraggerGrid = AuthoringSettings::singleton().getShowDraggerGrid();
    ARL::Draw::showHoverOver(AuthoringSettings::singleton().getShowHoverOver());
}
...
{
    boost::shared_ptr<ARL::DataModel> dataModel = m_CurrentGame->m_Game->getDataModel();

	dataModel->create<ARL::ContentProvider>()->setAssetFetchMediator(
		&UpdateUIManager::Instance().getViewWidget<RobloxGameExplorer>(eDW_GAME_EXPLORER));

	ARL::ServiceProvider::create<ARL::ServerScriptService>(dataModel.get());
	ARL::ServiceProvider::create<ARL::ServerStorage>(dataModel.get());
	ARL::ServiceProvider::create<ARL::ReplicatedStorage>(dataModel.get());
	...
	dataModel->create<ARL::LuaWebService>()->setCheckApiAccessBecauseInStudio();
	dataModel->create<ARL::Network::Players>()->setNonSuperSafeChatForAllPlayersEnabled(true);
...
void RobloxIDEDoc::displayWorkspaceMessage()
{
	...
	if (DFInt::StudioWorkspaceNotificationLevel && DFFlag::MaterialPropertiesEnabled )
	{
		...
		ARL::PhysicalPropertiesMode currentMode = this->getDataModel()->getWorkspace()->getPhysicalPropertiesMode();
		switch (DFInt::StudioWorkspaceNotificationLevel)
		{
		case 1:
			messageEnabled = currentMode == ARL::PhysicalPropertiesMode_Default;
			break;
		case 2:
			messageEnabled = currentMode <= ARL::PhysicalPropertiesMode_Default;
			break;
...
void RobloxIDEDoc::initializeNewPlace()
{
    ...
    bool oldStudioMode = ARL::GameBasicSettings::singleton().inStudioMode();
    ARL::GameBasicSettings::singleton().setStudioMode(false);

    // create instance for base plate
    boost::shared_ptr<ARL::Instance> instance = ARL::Creatable<ARL::Instance>::create<ARL::BasicPartInstance>();
    ARLASSERT(instance);
    ...
    partInstance->setColor(ARL::BrickColor::brickDarkGray());

    shared_ptr<ARL::DataModel> dataModel = m_CurrentGame->m_Game->getDataModel();
	...
	{
		ARLASSERT(!m_workspacePropertyChangedConnection.connected());
	...
	dataModel->getWorkspace()->insertInstances(
        ...
        NULL,
        ARL::INSERT_TO_TREE,
        ARL::SUPPRESS_PROMPTS);

    ARL::GameBasicSettings::singleton().setStudioMode(oldStudioMode);

    // zoom to a good spot
    ARL::Vector3 boxSize(15,15,15);
    ARL::Extents extents(-boxSize,boxSize);
    ARL::Camera* camera = dataModel->getWorkspace()->getCamera();
    camera->zoomExtents(extents,ARL::Camera::ZOOM_IN_OR_OUT);

	if(!FFlag::UserBetterInertialScrolling)
	{
		// adjust focus - zoom extents sets us in orbit cam
		ARL::CoordinateFrame frame = camera->getCameraCoordinateFrame();
		ARL::Vector3 look = frame.lookVector();
		camera->setCameraFocus(frame.translation + look * 2.0f);
	}

	// merge baseplate with change history baseline so we don't get an undo
    ARL::ChangeHistoryService* chs = dataModel->create<ARL::ChangeHistoryService>();
	...
	{
		ARLASSERT(!m_dataModelItemChangedConnection.connected());
...
void RobloxIDEDoc::initializeDataModeHash()
{
	...
	{
		ARL::DataModel::LegacyLock lock(getDataModel(), ARL::DataModelJob::Read);
...
void RobloxIDEDoc::initializeRobloxView()
{
    try
    {
        ARL::DataModel::LegacyLock lock(m_CurrentGame->m_Game->getDataModel(), ARL::DataModelJob::Write);
    ...
    {
        ARL::Log::current()->writeEntry(ARL::Log::Information,ARL::format("\tException : %s",e.what()).c_str());
...
bool RobloxIDEDoc::openFile(const QString& fileName, bool asNew)
{
    if (!fileName.isEmpty())
    {
        ...
        std::ifstream stream(ARL::utf8_decode(fileNameUtf8).c_str(), std::ios_base::in | std::ios_base::binary);
	...
	bool* abort, QDialog* dialog)
{
	...
	while (!*abort)
	{
		...
		if (boost::optional<int> status = p.get<int>("status"))
		{
			...
			{
				config->setFromValueTable(p.get<shared_ptr<const ARL::Reflection::ValueTable> >("settings").get());
...
bool RobloxIDEDoc::openStream(const QString& fileName, std::istream* stream, bool asNew)
{
    ...
    ARL::Log::current()->writeEntry(ARL::Log::Information,"RobloxIDEDoc::open - start");
    ...
    ARL::BaldPtr<QProgressBar> progressBar = UpdateUIManager::Instance().showProgress("Opening Place",0,ProgressStages);
	...
	try 
	{
        ...
        {
			m_EditGame.m_Game.reset(new ARL::UnsecuredStudioGame(NULL, baseUrl.toStdString().c_str(), StudioUtilities::isAvatarMode()));
        }
#else
        m_EditGame.m_Game.reset(new ARL::UnsecuredStudioGame(NULL,baseUrl.toStdString().c_str(), true));
#endif

		ARL::Log::current()->writeEntry(ARL::Log::Information,"\tLocking DataModel");
		ARL::DataModel::LegacyLock lock(m_EditGame.m_Game->getDataModel(), ARL::DataModelJob::Write);
		...
		if (graphicsMode != ARL::CRenderSettings::NoGraphics)
		{
            ...
            ARL::Log::current()->writeEntry(ARL::Log::Information,"\tInitializing View");
        ...
        ARL::Log::current()->writeEntry(ARL::Log::Information,"\tInitializing Explorer");
        ...
        ARL::Log::current()->writeEntry(ARL::Log::Information,ARL::format("\tLoading File : %s",qPrintable(fileName)).c_str());
            ...
            if (!m_initializationScript.isEmpty())
            {
				...
				else if (ARL::ContentProvider::isUrl(m_initializationScript.toStdString()))
                {
					...
					if (StudioUtilities::containsJoinScript(m_initializationScript))
                    {
						...
						{
							if (ARL::Network::Players* players = ARL::ServiceProvider::create<ARL::Network::Players>(getDataModel()))
							{
		...
		{
			ARL::Log::current()->writeEntry(ARL::Log::Information,"\tInitializing Terrain");
            ...
            ARL::Log::current()->writeEntry(ARL::Log::Information,"\tInitializing Undo");
            enableUndo(m_pMainWindow->getBuildMode() == BM_ADVANCED);

			// enable dynamic lighting for new places
			if (fileName.isEmpty())
			{
				if (ARL::Lighting* lighting = ARL::ServiceProvider::create<ARL::Lighting>(m_EditGame.m_Game->getDataModel().get()))
				{
            ...
            if (fileName.isEmpty())
            {
                ARL::MegaClusterInstance* terrain = boost::polymorphic_downcast<ARL::MegaClusterInstance*>(m_EditGame.m_Game->getDataModel()->getWorkspace()->getTerrain());
        ...
        ARL::Log::current()->writeEntry(ARL::Log::Information,"\tLoading Plugins");
		...
		ARL::UserInputService* userInputService = m_EditGame.m_Game->getDataModel()->find<ARL::UserInputService>();
        ...
        {
            ARL::Log::current()->writeEntry(ARL::Log::Information,"\tPrefetch Resources");
			preFetchResources();
        }
        ...
        ARL::Log::current()->writeEntry(ARL::Log::Information,"\tOpen Success");
	}
	catch(CreatePopupException &e)
	{
        ARL::Log::current()->writeEntry(ARL::Log::Information,ARL::format("\tPopup Exception : %s",qPrintable(e.m_message)).c_str());
    ...
    ARL::Log::current()->writeEntry(ARL::Log::Information,"RobloxIDEDoc::open - end");
	return success;
}
...
void RobloxIDEDoc::handleFileAction(FW::WatchID watchid, const FW::String& dir, const FW::String& filename, FW::Action action)
{
    ...
    ARL::ViewBase* pView = (&m_CurrentGame->m_View->getViewBase());
...
void RobloxIDEDoc::exportSelection(std::string filePath, ExporterSaveType exportType)
{
	...
	if (!fileName.isEmpty())
	{
		...
		if (fileInfo.suffix() == "obj")
		{
			...
			UpdateUIManager::Instance().setBusy(true);
			ARL::BaldPtr<QProgressBar> progressBar = UpdateUIManager::Instance().showProgress(exportAction);
...
void RobloxIDEDoc::onDMItemChanged(shared_ptr<ARL::Instance> item, const ARL::Reflection::PropertyDescriptor* descriptor)
{
	if (descriptor == &ARL::PartOperation::prop_CollisionFidelity)
	{
		if (PartOperation* partOp = item->fastDynamicCast<PartOperation>())
		{
			if (CSGDictionaryService* dictionaryService = ServiceProvider::create<CSGDictionaryService>(ARL::DataModel::get(partOp)))
			{
				if (!ARL::DataModel::get(partOp)->isRunMode())
				{
...
{
    //undo redo manager.
	ARL::ChangeHistoryService* chs = m_CurrentGame->m_Game->getDataModel()->create<ARL::ChangeHistoryService>();
	chs->setEnabled(true);
...
IRobloxDoc::ARLCloseRequest RobloxIDEDoc::requestClose()
{
	ARL::DataModel::RequestShutdownResult shutdownRequestResult;
	{
		ARL::DataModel::LegacyLock lock(m_EditGame.m_Game->getDataModel(), ARL::DataModelJob::Write);
	...
	{
		case ARL::DataModel::CLOSE_REQUEST_HANDLED:
			return IRobloxDoc::REQUEST_HANDLED;
		case ARL::DataModel::CLOSE_NOT_HANDLED:
			break;
		case ARL::DataModel::CLOSE_NO_SAVE_NEEDED:
            if (!m_TestModeEnabled)
                return IRobloxDoc::LOCAL_SAVE_NEEDED;
			return IRobloxDoc::NO_SAVE_NEEDED;
		case ARL::DataModel::CLOSE_LOCAL_SAVE:
			return IRobloxDoc::LOCAL_SAVE_NEEDED;
	}
...
bool RobloxIDEDoc::doClose()
{
	...
	{
		ARL::Scripting::DebuggerManager::singleton().setDataModel(NULL);
		DebuggerClientManager::Instance().setDataModel(boost::shared_ptr<ARL::DataModel>());
	...
	if (m_pRSFHandler)
    {
		shared_ptr<ARL::DataModel> dataModel = getEditDataModel();
	...
	{
		m_pDiagViewWidget->setDataModel(boost::shared_ptr<ARL::DataModel>());
...
void RobloxIDEDoc::cleanupGameState(GameState& gameState)
{
	...
	if (gameState.m_Game && gameState.m_Game->getDataModel())
    {
        ...
        // clean up verbs
        shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
		ARL::DataModel::LegacyLock lock(dataModel,ARL::DataModelJob::Write);	
		QVector<ARL::Verb*>::iterator iter = gameState.m_Verbs.begin();
    ...
    if (gameState.m_Game && gameState.m_Game->getDataModel())
	{
		...
		ARL::PluginManager::singleton()->deletePlugins(dataModel.get());
...
bool RobloxIDEDoc::loadFromStream(std::istream* stream)
{
    ...
    ARL::Security::Impersonator impersonate(ARL::Security::COM);
...
{
    ARL::Verb* verb = getVerb(m_EditGame,"publishToRobloxAsAction");
	if(verb) {
...
bool RobloxIDEDoc::isModified()
{
	...
	ARL::DataModel::LegacyLock lock(dataModel, ARL::DataModelJob::Read);
...
{
	if (!m_PlayGame.m_Game || !m_PlayGame.m_Game->getDataModel() || !m_PlayGame.m_Game->getDataModel()->create<ARL::RunService>())
		return false;

	ARL::RunState runState = m_PlayGame.m_Game->getDataModel()->create<ARL::RunService>()->getRunState();
	return runState != ARL::RS_STOPPED;
}
...
void RobloxIDEDoc::createScriptMapping(shared_ptr<ARL::Instance> instance)
{
	if (ARL::Instance::fastSharedDynamicCast<ARL::Script>(instance) || ARL::Instance::fastSharedDynamicCast<ARL::ModuleScript>(instance))
	{
...
void RobloxIDEDoc::restoreEditModelScripts()
{
	...
	{
		shared_ptr<ARL::Instance> playInstance = (*iter)->getCurrentScript().toInstance();
		if (playInstance == NULL)
		{
			continue;
		}

		shared_ptr<ARL::Instance> instance = getEditScriptByPlayInstance(playInstance.get());
		...
		else
		{
			...
			if ((*iter) == RobloxDocManager::Instance().getCurrentDoc())
			{
				...
				ARL::ScopedAssign<bool> ignoreDocActivate(m_bActive, true);
...
void RobloxIDEDoc::patchPlayModelScripts()
{
    ...
    ARL::DataModel* dm = m_EditGame.m_Game->getDataModel().get();
	...
	for (tScriptDocs::const_iterator iter = openScriptDocs.begin(); iter != openScriptDocs.end() ; ++iter)
	{
		shared_ptr<ARL::Instance> backingInstance = (*iter)->getCurrentScript().toInstance();
		...
		shared_ptr<ARL::Instance> instance = getScriptByIndexHierarchy(m_PlayGame.m_Game->getDataModel(), indexHierarchy, serviceName);
...
bool RobloxIDEDoc::saveScriptsChangedWhileTesting()
{
    ...
    if (playModifiedScriptList.size() > 0 && !isCloudEditSession())
    {
        ...
        if (saveChanges)
        {
            ...
            {
                shared_ptr<ARL::Instance> instance = getEditScriptByPlayInstance((*iter)->instance.get());
...
bool RobloxIDEDoc::saveAs(const QString& fileName)
{
	...
	shared_ptr<ARL::Game> game = m_EditGame.m_Game;
...
void RobloxIDEDoc::activate()
{
	...
	{
        ARL::PluginManager::singleton()->disableStudioUI(dataModel.get(), false);
	}
	ARL::PluginManager::singleton()->setCurrentDataModel(dataModel.get());
	...
	setDraggerGridMode((ARL::DRAG::DraggerGridMode)settings.value(DraggerGridModeSetting, ARL::DRAG::ONE_STUD).toInt());
...
void RobloxIDEDoc::deActivate()
{
	...
	if (!m_pMainWindow->isRibbonStyle()) // We never hide the plugins on ribbon mode
	{
        ARL::PluginManager::singleton()->disableStudioUI(m_CurrentGame->m_Game->getDataModel().get(),true);
	}
	...
	// save settings
    QSettings settings;
    settings.setValue(DraggerGridModeSetting,ARL::AdvArrowTool::advGridMode);
...
void RobloxIDEDoc::setDockViewsEnabled(bool state)
{
    ...
    shared_ptr<ARL::DataModel> dataModel = state ? m_CurrentGame->m_Game->getDataModel() : shared_ptr<ARL::DataModel>();
...
{
    ARLASSERT(m_CurrentGame->m_Game); 
    ARLASSERT(m_CurrentGame->m_Game->getDataModel());
...
void RobloxIDEDoc::mapActionIDWithVerb(GameState& gameState,const QString &actionID, const ARL::Name &verbName)
{
	gameState.m_VerbMap[actionID] = &verbName;
}

void RobloxIDEDoc::mapActionIDWithVerb(GameState& gameState,const QString &actionID, const ARL::Verb *pVerb)
{
	if (pVerb)
...
{
	if (verbName)
		mapActionIDWithVerb(gameState, actionID, ARL::Name::lookup(verbName));
}
...
bool RobloxIDEDoc::doHandleAction(const QString& actionID, bool isChecked)
{
	...
	try
	{
		...
		{
			ARL::Verb *pVerb = dataModel->getWorkspace()->getVerb(*m_CurrentGame->m_VerbMap[actionIDToLookFor]);
	...
	{
		RobloxSettings settings;
		settings.setValue(ARL_MANUAL_JOINT_CREATION_KEY, ARL::AdvArrowTool::advManualJointMode);
	}
...
void RobloxIDEDoc::preProcessVerb(const QStringList &tokens)
{
	...
	{
		ARL::MaterialTool::material = ARL::MaterialVerb::parseMaterial(tokens[1].toStdString());
	}
}
...
bool RobloxIDEDoc::handlePluginAction(void *pNotifier, void *pAction) 
{
	if (!pAction || !pNotifier)
		return false;
	static_cast<ARL::IHostNotifier*>(pNotifier)->buttonClick(m_CurrentGame->m_Game->getDataModel().get(), pAction);
	return true;
}

ARL::Reflection::Variant RobloxIDEDoc::evaluateCommandBarItem(const char* itemToEvaluate, shared_ptr<ARL::LuaSourceContainer> script)
{
    shared_ptr<ARL::DataModel> dataModel = m_CurrentGame->m_Game->getDataModel();
	ARL::DataModel::LegacyLock lock(dataModel,ARL::DataModelJob::Write);

    boost::shared_ptr<ARL::ScriptContext> pScriptContext = ARL::shared_from(dataModel->create<ARL::ScriptContext>());
    if(pScriptContext)
    {
        return pScriptContext->evaluateStudioCommandItem(itemToEvaluate, script);
    }
    return ARL::Reflection::Variant();
}
...
void RobloxIDEDoc::handleScriptCommand(const QString &execCommand)
{
	...
	try
	{
		...
		if (ARL::ContentProvider::isHttpUrl(command))
		{ 
			if (StudioUtilities::containsJoinScript(execCommand))
				isJoinScript = true;

			std::auto_ptr<std::istream> stream = 
                ARL::ServiceProvider::create<ARL::ContentProvider>(dataModel.get())->getContent(ARL::ContentId(command));
		...
		if (isJoinScript)
		{
			...
			{
				m_CurrentGame->m_Game->configurePlayer(ARL::Security::CmdLine_, command.substr(firstNewLineIndex+2));
				return;
			}
		}
		...
		boost::shared_ptr<ARL::ScriptContext> pScriptContext = ARL::shared_from(dataModel->create<ARL::ScriptContext>());
		if (pScriptContext)
			pScriptContext->executeInNewThread(ARL::Security::CmdLine_, ARL::ProtectedString::fromTrustedSource(command.c_str()), command.c_str());
	}
	catch (std::exception& e)
	{
		ARL::StandardOut::singleton()->print(ARL::MESSAGE_ERROR, e.what());
...
{		
	...
	{	
		ARL::Verb* pVerb = m_CurrentGame->m_Game->getDataModel()->getWorkspace()->getVerb(*m_CurrentGame->m_VerbMap[actionID]);
		if (pVerb)
		{
...
{
    shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
    QVector<ARL::Verb*>& verbs = gameState.m_Verbs;
	...
	verbs.append(new ARL::BoundVerb(dataModel.get(), "renameObjectVerb",
...
{
    boost::shared_ptr<ARL::CommonVerbs> commonVerbs = gameState.m_Game->commonVerbs;
    shared_ptr<ARL::DataModel> dataModel = gameState.m_Game->getDataModel();
    QVector<ARL::Verb*>& verbs = gameState.m_Verbs;
...
void RobloxIDEDoc::mapAdvToolsToolbar(GameState& gameState)
{
	...
	if (m_pMainWindow->isRibbonStyle())
	{
		...
		ARL::RightMotorTool::isStickyVerb = false;
		ARL::HingeTool::isStickyVerb = false;
...
void RobloxIDEDoc::mapRunToolbar(GameState& gameState)
{
	...
	//Run
	verbs.append(new ARL::BoundVerb(
        dataModel.get(), 
        "IDE Run",
...
void RobloxIDEDoc::mapAdvanceBuildToolbar(GameState& gameState)
{
	...
	{
		RobloxSettings settings;
		ARL::AdvArrowTool::advManualJointMode = settings.value(ARL_MANUAL_JOINT_CREATION_KEY, false).toBool();
...
void RobloxIDEDoc::onIdePause()
{
	...
	{
		shared_ptr<ARL::DataModel> playDataModel = m_PlayGame.m_Game->getDataModel();
		ARL::DataModel::LegacyLock lock(playDataModel,ARL::DataModelJob::Write);
		ARL::RunService* pRS = playDataModel->create<ARL::RunService>();
		pRS->pause();
	}
	else
	{
		shared_ptr<ARL::DataModel> dataModel = m_EditGame.m_Game->getDataModel();
		ARL::DataModel::LegacyLock lock(dataModel,ARL::DataModelJob::Write);
		ARL::RunService* pRS = dataModel->create<ARL::RunService>();
		pRS->pause();
	}
}
...
bool RobloxIDEDoc::isIdePauseEnabled()
{
	if (m_PlayGame.m_Game)
	{
		shared_ptr<ARL::DataModel> playDataModel = m_PlayGame.m_Game->getDataModel();
		ARL::RunService* pRS = playDataModel->create<ARL::RunService>();
		ARL::RunState runState = pRS->getRunState();
		return runState == ARL::RS_RUNNING;	
	}
...
void RobloxIDEDoc::teleportToURL(QString url, bool play)
{
    ...
    // Check to see if the url fetched returns anything at all.
    {
        ...
        std::auto_ptr<std::istream> stream = ARL::ServiceProvider::create<ARL::ContentProvider>(dataModel.get())->getContent(ARL::ContentId(command));
...
void RobloxIDEDoc::loadPlayDataModel(QString url, bool play, bool cloneDataModel, bool isTeleport)
{
...

	{
		...
		ARL::RunService* pRS = dataModel->create<ARL::RunService>();
		ARL::DataModel::LegacyLock editLock(dataModel,ARL::DataModelJob::Write);
		if (!pRS->isEditState())
		{
			applyScriptChanges();
			pRS->run();
    ...
    if (m_PlayGame.m_Game &&
        ...
        m_PlayGame.m_Game->getDataModel()->create<ARL::RunService>())
    {
		{	
			ARL::DataModel::LegacyLock lock(m_PlayGame.m_Game->getDataModel(),ARL::DataModelJob::Write);
			ARL::RunState runState = m_PlayGame.m_Game->getDataModel()->create<ARL::RunService>()->getRunState();
			if (runState == ARL::RS_PAUSED)
			{
				ARL::ServiceProvider::create<ARL::RunService>(m_PlayGame.m_Game->getDataModel().get())->run();
				return;
			}
		}
    }
    ...
    {
		{
			ARL::RunService* pRunState = m_EditGame.m_Game->getDataModel()->find<ARL::RunService>();
			ARL::DataModel::LegacyLock lock(m_EditGame.m_Game->getDataModel(),ARL::DataModelJob::Write);
			if(pRunState && pRunState->getRunState() == ARL::RS_PAUSED)
			{				
				pRunState->run();
				return;
			}
    ...
    ARL::BaldPtr<QProgressBar> progressBar = UpdateUIManager::Instance().showProgress("Play Game",0,ProgressStages);
    ...
    if (m_pMainWindow->isRibbonStyle())
    {
        ARL::PluginManager::singleton()->hideStudioUI(m_EditGame.m_Game->getDataModel().get(),true);
    }
    ...
    if (!m_PlayGame.m_Game)
    {
        ...
        QString baseUrl = RobloxSettings::getBaseURL() + "/";
        m_PlayGame.m_Game.reset(new ARL::UnsecuredStudioGame(NULL,baseUrl.toStdString().c_str()));
        ...
        ARL::UserInputService* userInputService = playDataModel->find<ARL::UserInputService>();
        userInputService->setMouseEnabled(true);
        userInputService->setKeyboardEnabled(true);
    }
    ...
    if (cloneDataModel)
    {
        ARL::DataModel::LegacyLock editLock(dataModel,ARL::DataModelJob::Write);
        applyScriptChanges();
        ARL::SerializerBinary::serialize(ss, dataModel.get());
    ...
    shared_ptr<ARL::DataModel> playDataModel = m_PlayGame.m_Game->getDataModel();
    {
        ARL::DataModel::LegacyLock playLock(playDataModel,ARL::DataModelJob::Write);
        ...
        if (cloneDataModel)
        {
            ARL::SerializerBinary::deserialize(ss, playDataModel.get());       
		...
		for (tDebuggersMap::const_iterator iter = m_EditDebuggersMap.begin(); iter != m_EditDebuggersMap.end(); ++iter)
		{
			...
			shared_ptr<ARL::Instance> instance = getScriptByIndexHierarchy(playDataModel, indexHierarchy, serviceName);
    ...
    if (play)
    {
        ARLASSERT(m_CurrentGame == &m_PlayGame);
    ...
    {
        ARL::DataModel::LegacyLock playLock(playDataModel,ARL::DataModelJob::Write);
        ARL::ServiceProvider::create<ARL::RunService>(playDataModel.get())->run();
    }
...
void RobloxIDEDoc::closePlayDataModel()
{
    ...
    if (FFlag::LuaDebugger)
		DebuggerClientManager::Instance().onRunTransition(ARL::RunTransition(ARL::RS_STOPPED, ARL::RS_STOPPED));
    ...
    if (m_pMainWindow->isRibbonStyle())
    {
        ARL::PluginManager::singleton()->hideStudioUI(m_CurrentGame->m_Game->getDataModel().get(), false);
    }
...
void RobloxIDEDoc::onIdeRun(bool play)
{
	...
	if (DFFlag::ScriptExecutionContextApi)
	{
		if (!play)
		{
			if (shared_ptr<ARL::DataModel> playDataModel = m_PlayGame.m_Game->getDataModel())
			{
...
bool RobloxIDEDoc::isIdeRunEnabled(bool play)
{
	if (m_PlayGame.m_Game) 
	{
		...
		return pRS->getRunState() != ARL::RS_RUNNING;
	}
    else
    {
        shared_ptr<ARL::DataModel> dataModel = m_EditGame.m_Game->getDataModel();
        ARL::RunService* pRS = dataModel->create<ARL::RunService>();
		bool clientIsPresent = ARL::Network::Players::clientIsPresent(dataModel.get()) &&
			!isCloudEditSession();
	    return !clientIsPresent && (pRS->getRunState() != ARL::RS_RUNNING);
    }
}
...
ARL::Verb* RobloxIDEDoc::getVerb(const QString& name) const
{
	ARLASSERT(m_CurrentGame);
	return getVerb(*m_CurrentGame, name);
}

ARL::Verb* RobloxIDEDoc::getVerb(GameState& gameState,const QString& name) const
{
...
bool RobloxIDEDoc::autoSave(bool force)
{
    ...
    try
    {
        ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO,"Auto-Saving...");
...
void RobloxIDEDoc::resetDirty(ARL::DataModel* dataModel)
{
	if (!dataModel)
		return;
...
{
    ARL::Verb* verb = m_CurrentGame->m_Game->getDataModel()->getVerb(stats.toStdString());
...
bool RobloxIDEDoc::displayAskConvertPlaceToNewMaterialsIfInsertNewModel()
{
	...
	if (!conversionDialogDisabled)
	{
		...
		QtUtilities::ARLConfirmationMessageBox msgBox(
...
void RobloxIDEDoc::onWorkspacePropertyChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor)
{
	if (pDescriptor->name == ARL::Workspace::prop_physicalPropertiesMode.name)
	{
		...
		if (getEditDataModel()->getWorkspace()->getUsingNewPhysicalProperties())
		{
			...
			{
				ARL::PartInstance::convertToNewPhysicalPropRecursive(dm);
			}
		}
	}
}

static void createDebuggerClient(shared_ptr<ARL::Instance> instance, ARL::Scripting::DebuggerManager* pDebuggerManager)
{
	if (!instance || instance->isDescendantOf(ARL::ServiceProvider::find<ARL::CoreGuiService>(instance.get())))
		return;

	shared_ptr<ARL::Script> pScript = ARL::Instance::fastSharedDynamicCast<ARL::Script>(instance);
	if (pScript)
	{
		//ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO, "Script Name: %s", pScript->getName().c_str());
		pDebuggerManager->addDebugger(pScript.get());
	}
	else
	{
		shared_ptr<ARL::ModuleScript> pModuleScript = ARL::Instance::fastSharedDynamicCast<ARL::ModuleScript>(instance);
		if (pModuleScript)
			pDebuggerManager->addDebugger(pModuleScript.get());
	}
}

void RobloxIDEDoc::initializeDebuggerData(const QString &debugFile, const shared_ptr<ARL::DataModel> dm, const tDebuggersMap& debuggersMap)
{
	...
	if (dm != DebuggerClientManager::Instance().getDataModel())
		DebuggerClientManager::Instance().setDataModel(shared_ptr<ARL::DataModel>());
	...
	else
	{
		...
		if (debugFileInfo.exists())
		{
			...
			try
			{
				ARL::SerializerBinary::deserialize(stream, &debuggerManager);
			}
 ...
 */
void RobloxIDEDoc::setDraggerGridMode(ARL::DRAG::DraggerGridMode gridMode)
{
	if ( gridMode == ARL::DRAG::ONE_STUD )
		m_pMainWindow->gridToOneAction->activate(QAction::Trigger);
	else if ( gridMode == ARL::DRAG::QUARTER_STUD )
...
void RobloxIDEDoc::placeNameLoaded(QString json)
{
	...
	if (ARL::WebParser::parseJSONObject(json.toStdString(), v))
	{
...
{
	ARLASSERT(m_EditGame.m_Game->getDataModel()->write_requested);
	...
	if (FFlag::SearchToolboxByDataModelSearchString)
		if (Instance* defaultToolboxSearchInstance = m_EditGame.m_Game->getDataModel()->findFirstChildByName("DefaultToolboxSearch"))
			if (StringValue* stringValue = ARL::Instance::fastDynamicCast<StringValue>(defaultToolboxSearchInstance))
			{
...
QString RobloxIDEDoc::getUploadURL()
{
	...
	{
		if(ARL::Visit* pVisit = ARL::ServiceProvider::find<ARL::Visit>(m_CurrentGame->m_Game->getDataModel().get()))
...
void RobloxIDEDoc::notifyScriptEdited(shared_ptr<ARL::Instance> script)
{
	if (script)
	{
...
void RobloxIDEDoc::onPlayModifiedAncestryChanged(shared_ptr<ARL::Instance> script, shared_ptr<ARL::Instance> newParent)
{
	if (newParent==NULL)
	{
...
void RobloxIDEDoc::addScriptMapping(shared_ptr<ARL::Instance> script, shared_ptr<ARL::DataModel> dataModel)
{
	if (!script)
		return;
    ...
    shared_ptr<ARL::Instance> instance = getScriptByIndexHierarchy(dataModel, indexHierarchy, serviceName);
	if (instance)
		m_scriptMap[instance.get()] = script;
}

std::string RobloxIDEDoc::getScriptIndexHierarchy(shared_ptr<ARL::Instance> script,  std::vector<int>& indexHierarchy)
{
	if (!script)
		return "";
	...
	while (pParent)
	{
        ...
        if (pParent && dynamic_cast<const ARL::Service*>(pParent) &&
            pParent->getParent() &&
...
shared_ptr<ARL::Instance> RobloxIDEDoc::getScriptByIndexHierarchy(shared_ptr<ARL::DataModel> dataModel, const std::vector<int>& indexHierarchy, const std::string& serviceName)
{
    if (serviceName == "")
        return shared_ptr<ARL::Instance>();

    shared_ptr<ARL::Instance> pParent = shared_from(dataModel->findFirstChildOfType(serviceName));
	shared_ptr<ARL::Instance> pChild;
	...
	if (pChild && (pChild->isA<ARL::Script>() || pChild->isA<ARL::ModuleScript>()))
		return pChild;
	return shared_ptr<ARL::Instance>();
}

boost::shared_ptr<ARL::Instance> RobloxIDEDoc::getEditScriptByPlayInstance(ARL::Instance* instance)
{
...
void RobloxIDEDoc::updateEditModelDebuggerData()
{
	...
	{
		ARL::DataModel* dm = m_PlayGame.m_Game->getDataModel().get();
...
void RobloxIDEDoc::updateDebuggerManager(const tDebuggersMap& debuggers)
{
	...
	ARL::Scripting::DebuggerManager& debuggerManager = ARL::Scripting::DebuggerManager::singleton();
	boost::shared_ptr<ARL::Scripting::ScriptDebugger> debugger;
	boost::shared_ptr<ARL::Scripting::DebuggerBreakpoint> breakpoint;
	...
	for (tDebuggersMap::const_iterator iter = debuggers.begin(); iter != debuggers.end(); ++iter)
	{
		...
		if (debugger)
		{
			...
			for (std::list<DebuggerData::BreakpointData>::const_iterator bp_iter = breakpoints.begin(); bp_iter != breakpoints.end(); ++bp_iter)
			{
				...
				if (breakpoint)
					ARL::Scripting::DebuggerBreakpoint::prop_Enabled.setValue(breakpoint.get(), bp_iter->enabled);
			}
...
void RobloxIDEDoc::addToDebuggersMap(shared_ptr<ARL::Instance> instance, tDebuggersMap& debuggersMap)
{
	if (ARL::Instance::fastSharedDynamicCast<ARL::Script>(instance) || ARL::Instance::fastSharedDynamicCast<ARL::ModuleScript>(instance))
	{
		ARL::Scripting::DebuggerManager& debuggerManager = ARL::Scripting::DebuggerManager::singleton();
		if (ARL::Scripting::ScriptDebugger* debugger = debuggerManager.findDebugger(instance.get()))
			debuggersMap[instance] = getDebuggerData(debugger);
	}
}

void RobloxIDEDoc::updateDebuggersMap(shared_ptr<ARL::Instance> instance, tDebuggersMap& debuggersMap)
{
	if (ARL::Instance::fastSharedDynamicCast<ARL::Script>(instance) || ARL::Instance::fastSharedDynamicCast<ARL::ModuleScript>(instance))
	{
		shared_ptr<ARL::Instance> editScript = getEditScriptByPlayInstance(instance.get());
		if (editScript)
		{
			ARL::Scripting::DebuggerManager& debuggerManager = ARL::Scripting::DebuggerManager::singleton();
			if (ARL::Scripting::ScriptDebugger* debugger = debuggerManager.findDebugger(instance.get()))
...
RobloxIDEDoc::DebuggerData RobloxIDEDoc::getDebuggerData(ARL::Scripting::ScriptDebugger* debugger)
{
	DebuggerData data;
...
bool RobloxIDEDoc::isEditMode(const ARL::Instance* context)
{
	...
	if (RobloxApplicationManager::instance().hasLocalServer())
		return false;
	return !ARL::Network::Players::clientIsPresent(context) && !ARL::Network::Players::findConstLocalPlayer(context);
}
...
void RobloxIDEDoc::forceReloadImages(const QStringList& contentIds)
{
	...
	ARL::ViewBase* pView = (&m_CurrentGame->m_View->getViewBase());
	shared_ptr<ARL::DataModel> dm(m_CurrentGame->m_Game->getDataModel());
	{
		ARL::DataModel::LegacyLock lock(dm, ARL::DataModelJob::Write);
		...
		pView->bindWorkspace(shared_ptr<ARL::DataModel>());
...
{
	if (ARL::HttpRbxApiService* apiService = ARL::ServiceProvider::find<ARL::HttpRbxApiService>(getDataModel()))
	{
		m_bIsLocalDocument = false;
		apiService->getAsync( ARL::format("marketplace/productinfo?assetId=%d", placeId),false, ARL::PRIORITY_DEFAULT,
...
void RobloxIDEDoc::resetMouseCommand(const char* mouseCommandToReset)
{
	...
	{
		ARL::Workspace* workspace = dataModel->getWorkspace();
		ARL::MouseCommand* mouseCommand = workspace->getCurrentMouseCommand();
		if (mouseCommand)
		{
...
void RobloxIDEDoc::setMuteAllSounds(bool mute)
{
	...
	if (boost::shared_ptr<ARL::DataModel> dataModel = m_CurrentGame->m_Game->getDataModel())
	{
		if (ARL::Soundscape::SoundService* soundService = ARL::ServiceProvider::create<ARL::Soundscape::SoundService>(dataModel.get()))
		{
...
QString RobloxIDEDoc::getRecentSaveFile()
{
	...
	if (QDir().mkpath(recentSavesPath))
	{
		...
		if (m_bIsLocalDocument)
		{
			fileToSave = recentSavesPath+"/"+baseName+"_"+ QString::number(ARL::Hash::hash(QFileInfo(fileName()).absoluteFilePath().toStdString()))+".rbxl";	
		}
		else
		{
...
void RobloxIDEDoc::onLocalPlayerAdded(shared_ptr<ARL::Instance> newPlayer)
{	
	m_localPlayerConnection = newPlayer->propertyChangedSignal.connect(boost::bind(&RobloxIDEDoc::onLocalPlayerNameChanged, this, _1));	
}

void RobloxIDEDoc::onLocalPlayerNameChanged(const ARL::Reflection::PropertyDescriptor* pDescriptor)
{
	if (*pDescriptor == ARL::Instance::desc_Name)
	{
...
{
	ARL::DataModel* datamodel = getDataModel();
	if (!datamodel)
		return;

	{
		ARL::DataModel::LegacyLock lock(datamodel, ARL::DataModelJob::Read);
		if (ARL::Network::Player* localPlayer = ARL::Network::Players::findLocalPlayer(datamodel))
...
QString RobloxIDEDoc::downloadPlaceCopy()
{
	...
	if (!fileName.isEmpty())
	{
		...
		if (result)
		{
			...
			ARL::StandardOut::singleton()->printf(ARL::MESSAGE_INFO, qPrintable(message));
			return fileName;
		}
	}
