...
Q_DECLARE_METATYPE(ARL::HttpFuture);
...
QVariant getEntityId(EntityProperties* settings, EntityCategory category) {
	...
	if (settings)
	{
		...
		ARLASSERT(intId.is_initialized() != stringId.is_initialized());
		if (intId)
		{
			id = intId.get();
		}
...
QString namedImagesDisplayNameGetter(EntityProperties* settings)
{
	...
	ARLASSERT(name.startsWith(kImageAssetNamePrefix));
...
void EntityProperties::setFromEntitySettings(EntityProperties& other)
{
	...
	ARLASSERT(!other.hasPendingFuture);
	ARLASSERT(other.ableToGetFuture);
	ARLASSERT(other.ableToParseJson);
...
boost::optional<Variant> EntityProperties::getVariant(const std::string& fieldName)
{
	...
	ARL::Reflection::ValueTable::iterator itr = data.find(fieldName);
	if (itr != data.end())
	{
		result = itr->second;
	}
	...
	boost::unordered_map<IdType, boost::function<IdType()> >* outputRemaps)
{
	...
	ARLASSERT(&webSettings == &target->webSettings);
			...
			!found && targetItr != target->data.end(); ++targetItr)
		{
				...
				targetItr->get<std::string>("Name").get() == name.get())
			{
				...
				for (size_t i = 0; i < ignoreFieldsWhenCopying.size(); ++i)
				{
					...
					ARLASSERT(valueToKeep.is_initialized());
...
boost::optional<std::string> RobloxGameExplorer::findCachedAssetOrEmpty(const ARL::ContentId& contentId, int universeId)
{
...
void RobloxGameExplorer::updateAsset(const ARL::ContentId& contentId, const std::string& newContents)
{
	ARLASSERT(contentId.isNamedAsset());
	ARLASSERT(currentGameId != kNoGameLoaded);
...
void RobloxGameExplorer::notifyIfUnpublishedChanges(int universeId)
{
	...
	{
		QMessageBox mb;
		mb.setText("ANORRL Studio is using unpublished changes for this universe. You can revert "
...
void RobloxGameExplorer::openGameFromGameId(int gameId)
{
	...
	if (gameId != currentGameId && ideDoc)
	{
		...
		ARLASSERT((currentPlaceId == kNoGameLoaded) == (currentGameId == kNoGameLoaded));
		...
		ARLASSERT(state == QMessageBox::Yes);
	...
	EntityProperties* properties)
{
	...
	ARLASSERT(sourceAssetTypeId == (int)ASSET_TYPE_ID_Script);
