...
//--------------------------------------------------------------------------------------------
class PropertyItemUpdateJob : public ARL::DataModelJob
{	
public:
...
public:
	...
	PropertyItemUpdateJob(PropertyTreeWidget *pPropertyWidget, shared_ptr<ARL::DataModel> dataModel)
	: ARL::DataModelJob("PropGrid", ARL::DataModelJob::Read, false, dataModel, ARL::Time::Interval(0))
...
void PropertyTreeWidget::setDataModel(boost::shared_ptr<ARL::DataModel> pDataModel)
{
...
void PropertyTreeWidget::restoreOriginalValues(PropertyItem* item)
{
	...
	ARL::DataModel::LegacyLock lock(getDataModel(), ARL::DataModelJob::Write);

	for (boost::unordered_map<shared_ptr<ARL::Instance>, ARL::Reflection::Variant>::const_iterator iter = m_originalValues.begin(); iter != m_originalValues.end(); ++iter)
...
void PropertyTreeWidget::storeOriginalValues(PropertyItem* item)
{
	...
	if (ARL::Selection* selection = ARL::ServiceProvider::create<ARL::Selection>(getDataModel().get()))
	{
		ARL::Instance* prevInstance = NULL;
...
void PropertyTreeWidget::addInstance(ARL::Instance *pInstance)
{
...
void PropertyTreeWidget::removeInstance(ARL::Instance *pInstance)
{	
	//remove connection
...
void PropertyTreeWidget::onPropertyChanged(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
...
void PropertyTreeWidget::update()
{
	...
	if (m_itemMap.empty()) 
	{
		visitClassProperties(&ARL::Reflection::ClassDescriptor::rootDescriptor());
...
void PropertyTreeWidget::visitClassProperties(const ARL::Reflection::ClassDescriptor* pClassDescriptor)
{
	//visit classes
	...
	std::for_each(pClassDescriptor->begin<ARL::Reflection::PropertyDescriptor>(), 
		          pClassDescriptor->end<ARL::Reflection::PropertyDescriptor>(), 
				  boost::bind(&PropertyTreeWidget::initProperty, this, pClassDescriptor, _1));
}

void PropertyTreeWidget::initProperty(const ARL::Reflection::ClassDescriptor* pClassDescriptor, 
										  const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
	...
	ARL::Reflection::Metadata::Member* pMember = ARL::Reflection::Metadata::Reflection::singleton()->get(*pPropertyDescriptor);

    // only show public, editable properties
    if ( !pPropertyDescriptor->isPublic() || 
         !pPropertyDescriptor->isEditable() || 
			(pPropertyDescriptor->security != ARL::Security::None && 
				!(ARL::Reflection::Metadata::Item::isBrowsable(pMember, *pPropertyDescriptor)) 
			)
		)
	{
		return;
	...
	if(ARL::Reflection::Metadata::Item::isDeprecated(pMember, *pPropertyDescriptor))
		return;
...
CategoryItem* PropertyTreeWidget::getCategoryItem(const ARL::Reflection::PropertyDescriptor* pPropertyDescriptor)
{
        ...
        if (treeWidgetItem)
		{
			if(FFlag::StudioNewWiki)
			{
				...
				while (!done) {
					...
					const ARL::Reflection::PropertyDescriptor* desc = pi->propertyDescriptor();
					if(desc) {
						done = true;
...
void RobloxPropertyWidget::setDataModel(boost::shared_ptr<ARL::DataModel> pDataModel)
{
	if ( m_pDataModel == pDataModel )
       return;
...
void RobloxPropertyWidget::addInstance(boost::shared_ptr<ARL::Instance> pInstance)
{
	if (!pInstance)
		return;
	
	//ARL::readwrite_concurrency_catcher::scoped_write_request lock(m_selectedInstancesGuard);
	m_selectedInstaces.push_back(pInstance.get());
	m_pTreeWidget->addInstance(pInstance.get());
}

void RobloxPropertyWidget::removeInstance(boost::shared_ptr<ARL::Instance> pInstance)
{
	if (!pInstance)
		return;
...
void RobloxPropertyWidget::onInstanceSelectionChanged(const ARL::SelectionChanged& evt)
{
...
void RobloxPropertyWidget::updateSelectionInfoLabel()
{
	...
	const ARL::Name* className = &(*curIter)->getClassName();
