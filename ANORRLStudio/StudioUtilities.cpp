...
namespace StudioUtilities
{
	...
	std::string authenticate(std::string &domain, std::string &url, std::string &ticket)
	{
		...
		try
		{
			...
			std::string result;
			{
				...
				//http.additionalHeaders["ARLAuthenticationNegotiation:"] = domain;
				http.get(result);
			}
	...
	ARL::ProtectedString fetchAndValidateScript(ARL::DataModel* pDataModel, const std::string& urlScript)
	{
		if(!pDataModel)
			return ARL::ProtectedString();

		ARL::Security::Impersonator impersonate(ARL::Security::COM);

		std::ostringstream data;
		if (!ARL::ContentProvider::isUrl(urlScript))
			return ARL::ProtectedString();
		...
		std::string dataString;
		{
			try 
			{
				...
				std::auto_ptr<std::istream> stream(ARL::ServiceProvider::create<ARL::ContentProvider>(pDataModel)->getContent(ARL::ContentId(urlScript.c_str())));
				dataString = std::string(static_cast<std::stringstream const&>(std::stringstream() << stream->rdbuf()).str());
			}

			catch(std::exception& e)
			{
				ARL::StandardOut::singleton()->printf(ARL::MESSAGE_ERROR, "ContentProvider while executing url script failed because %s", e.what());
			}
		}

		ARL::ProtectedString verifiedSource;

		try
		{
			verifiedSource = ARL::ProtectedString::fromTrustedSource(dataString);
			ARL::ContentProvider::verifyScriptSignature(verifiedSource, true);
		}
		catch(std::bad_alloc&)
		{
			throw;
		...
		{
			ARL::StandardOut::singleton()->printf(ARL::MESSAGE_ERROR, "Script signature verification failed -- %s", e.what());
			return ARL::ProtectedString();
		}
	...
	{
		ARLASSERT(containsJoinScript(QString::fromStdString(urlScript)));

		boost::shared_ptr<ARL::DataModel> pDataModel = pGame->getDataModel();
		if(!pDataModel)
			return;

		ARL::ProtectedString verifiedSource = fetchAndValidateScript(pDataModel.get(), urlScript);
		...
		{		
			...
			{
				pGame->configurePlayer(ARL::Security::COM, dataString.substr(firstNewLineIndex+2));
			...
			boost::shared_ptr<ARL::ScriptContext> pContext = ARL::shared_from(pDataModel->create<ARL::ScriptContext>());
			if (pContext)
				pContext->executeInNewThread(ARL::Security::COM, verifiedSource, "Start Game");	
		}
		catch (std::exception& exp)
		{
			std::string s(verifiedSource.getSource());
			std::string msg = ARL::format("executeURLJoinScript() failed because %s.  %s", exp.what(), s.substr(0,64).c_str());
			ARL::StandardOut::singleton()->print(ARL::MESSAGE_ERROR, msg.c_str()); 
		}
	}
	...
	void executeURLScript(boost::shared_ptr<ARL::DataModel> pDataModel, std::string urlScript)
	{
		...
		{
			std::string s(verifiedSource.getSource());
			std::string msg = ARL::format("executeURLScript() failed because %s.  %s", exp.what(), s.substr(0,64).c_str());
			ARL::StandardOut::singleton()->print(ARL::MESSAGE_ERROR, msg.c_str()); 
		}
	}
	...
	static void updateCameraLocation(ARL::DataModel* dm)
	{
		if (ARL::Network::Player* p = ARL::Network::Players::findLocalPlayer(dm))
		{
			p->setCloudEditCameraCoordinateFrame(dm->getWorkspace()->getCamera()->getCameraCoordinateFrame());
		}
	}

	static void doUpdateSelection(shared_ptr<bool> debouncer, ARL::DataModel* dm)
	{
		if (ARL::Network::Player* p = ARL::Network::Players::findLocalPlayer(dm))
		{
			if (ARL::Selection* s = dm->find<ARL::Selection>())
			{
				shared_ptr<ARL::Reflection::ValueArray> va(new ARL::Reflection::ValueArray);
				for (auto& i : *s)
				{
					va->push_back(i);
				}
				ARL::Network::Player::event_cloudEditSelectionChanged.replicateEvent(p, shared_ptr<const ARL::Reflection::ValueArray>(va));
			}
		}
	...
	static void updateSelection(shared_ptr<bool> debouncer, ARL::DataModel* dm)
	{
		if (FFlag::TeamCreateOptimizeRemoteSelection)
		{
			if (*debouncer) return;
			*debouncer = true;
			dm->submitTask(boost::bind(&doUpdateSelection, debouncer, _1), ARL::DataModelJob::Write);
		}
		else
		{
	...
	void joinCloudEditSession(shared_ptr<ARL::Game> pGame, shared_ptr<EntityProperties> config)
	{
		using namespace ARL;
		try
		{
			...
			ARL::Analytics::setUserId(userId);
		...
		catch (const std::exception& e) 
		{
			...
			QtUtilities::ARLMessageBox failureBox;
    ...
    void convertPhysicalPropertiesIfNeeded(std::vector<boost::shared_ptr<ARL::Instance> > instances, ARL::Workspace* workspace)
	{
		...
		else 
		{
			for (size_t i = 0; i < instances.size(); i++)
			{
				if (ARL::PartInstance::instanceOrChildrenContainsNewCustomPhysics(instances[i].get())) 
				{
	...
	void insertModel(boost::shared_ptr<ARL::DataModel> pDataModel, QString fileName, bool insertInto)
	{
		if (fileName.isEmpty())
			return;
		...
		
		{
			...
			if (ARL::ContentProvider* cp = pDataModel->create<ARL::ContentProvider>())
			{
		...
		if (!insertInto){
			if (instances.size() == 1)
			{
				...
				if (dynamic_cast<ARL::Tool*>(single)) 
				{
					if( QtUtilities::ARLMessageBox::question(
		...
		shared_ptr<ARL::Workspace> spWorkspace = shared_from(pDataModel ? pDataModel->getWorkspace() : NULL);
		if(!spWorkspace)
			throw std::runtime_error("Can't insert at this time");
		
		ARL::Selection* sel = ARL::ServiceProvider::create<ARL::Selection>(spWorkspace.get());
		
		ARL::Instance* requestedParent = (insertInto && (sel->size() == 1))
		? sel->front().get()
		: spWorkspace.get();
		
		ARL::InsertMode insertMode = insertInto ? ARL::INSERT_TO_TREE : ARL::INSERT_TO_3D_VIEW;
	...
	void insertScript(boost::shared_ptr<ARL::DataModel> pDataModel, const QString &fileName)
	{
		shared_ptr<ARL::Workspace> spWorkspace = shared_from(pDataModel ? pDataModel->getWorkspace() : NULL);
		...
		shared_ptr<ARL::Script> pScriptInstance = ARL::Creatable<ARL::Instance>::create<ARL::Script>();
		if (!pScriptInstance)
		...
		ARL::Instance*  pRequestedParent = (pSelection && pSelection->size() == 1) ? pSelection->front().get() : spWorkspace.get();
	...
	QString getDebugInfoFile(const QString& fileName, const QString& debuggerFileExt)
	{
		...
		if (!fileName.isEmpty())
		{
			static QString filePath = QtUtilities::qstringFromStdString(ARL::FileSystem::getUserDirectory(true, ARL::DirAppData, "debugger").native());
			// get hash of the file path
			unsigned int hash = ARL::Hash::hash(QFileInfo(fileName).absoluteFilePath().toStdString());
