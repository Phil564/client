...
// GroupItem
//--------------------------------------------------------------------------------------------
static void invokeGroupDataLoaded(QPointer<GroupItem> pGroupItem, ARL::HttpFuture future)
{
	if (pGroupItem.isNull())
		return;
	QMetaObject::invokeMethod(pGroupItem.data(), "groupDataLoaded", Qt::QueuedConnection, Q_ARG(ARL::HttpFuture, future));
}

static void invokeGroupThumbnailLoaded(QPointer<GroupItem> pGroupItem, ARL::HttpFuture future)
{
	if (pGroupItem.isNull())
		return;
	QMetaObject::invokeMethod(pGroupItem.data(), "groupThumbnailLoaded", Qt::QueuedConnection, Q_ARG(ARL::HttpFuture, future));
}
...
: m_GroupId(groupId)
{
	...
	ARL::HttpAsync::get(str.toStdString()).then(boost::bind(&invokeGroupDataLoaded, QPointer<GroupItem>(this), _1));
}

void GroupItem::groupDataLoaded(ARL::HttpFuture future)
{
	...
	if (thumnailUri.is_initialized() && !thumnailUri.get().empty())
		ARL::HttpAsync::get(thumnailUri.get()).then(boost::bind(&invokeGroupThumbnailLoaded, QPointer<GroupItem>(this), _1));
}

void GroupItem::groupThumbnailLoaded(ARL::HttpFuture future)
{
...
//--------------------------------------------------------------------------------------------
CreatorsListWidget::CreatorsListWidget(QWidget* parent, shared_ptr<ARL::DataModel> dataModel, shared_ptr<PlayersDataManager> dataManager)
: QTreeWidget(parent)
, m_pDataModel(dataModel)
, m_pPlayersDataManager(dataManager)
{
	ARLASSERT(m_pDataModel);
	ARLASSERT(m_pPlayersDataManager);
...
void CreatorsListWidget::onPlayerAdded(boost::shared_ptr<ARL::Instance> player)
{
	int playerId = ARL::Instance::fastDynamicCast<ARL::Network::Player>(player.get())->getUserID();
	if (playerId <= 0)
		return;
...
void CreatorsListWidget::onPlayerRemoving(boost::shared_ptr<ARL::Instance> player)
{
	int playerId = ARL::Instance::fastDynamicCast<ARL::Network::Player>(player.get())->getUserID();
	if (playerId <= 0)
		return;
...
QString CreatorsListWidget::getPlayerName(int playerId)
{
	...
	if (players)
	{
		boost::shared_ptr<ARL::Network::Player> player = players->getPlayerByID(playerId);
		if (player)
...
//--------------------------------------------------------------------------------------------
UserPlaceCreatorsListWidget::UserPlaceCreatorsListWidget(QWidget* parent, shared_ptr<ARL::DataModel> dataModel, shared_ptr<PlayersDataManager> dataManager)
: CreatorsListWidget(parent, dataModel, dataManager)
, m_PlaceOwnerId(-1)
{
	ARLASSERT(dataModel->getUniverseId());
...
//--------------------------------------------------------------------------------------------
GroupPlaceCreatorsListWidget::GroupPlaceCreatorsListWidget(QWidget* parent, shared_ptr<ARL::DataModel> dataModel, shared_ptr<PlayersDataManager> dataManager)
: CreatorsListWidget(parent, dataModel, dataManager)
{
	ARL::Network::Players* players = m_pDataModel->find<ARL::Network::Players>();
	if (players)
...
void RobloxTeamCreateWidget::setDataModel(boost::shared_ptr<ARL::DataModel> dataModel, boost::shared_ptr<PlayersDataManager> pPlayersDataManager)
{
...
void RobloxTeamCreateWidget::onAuthenticationChanged(bool)
{
	...
	{
		// this has to be delayed as user ARL::User authentication SLOT might not have been called by now
		QMetaObject::invokeMethod(this, "updateCurrentWidgetInStack", Qt::QueuedConnection);
	}
}
...
{
	ARLASSERT(m_pDataModel->getUniverseId() > 0);
...
void RobloxTeamCreateWidget::onBottomMoreButtonClicked()
{
	...
	if (resultAction == cloudEditTurnOffAction)
	{
		...
		ARLASSERT(m_pDataModel->getUniverseId());
...
void RobloxTeamCreateWidget::onPlaceIdChanged(const ARL::Reflection::PropertyDescriptor* desc)
{
	...
	m_PlaceManageStatusQuery = ARL::HttpAsync::get(canManageUrl.toStdString()).then(boost::bind(&RobloxTeamCreateWidget::updatePlaceManageStatus, this, _1));
}
...
void RobloxTeamCreateWidget::updatePlaceManageStatus(ARL::HttpFuture future)
{
	...
	m_PlaceDetailsQuery = ARL::HttpAsync::get(url.toStdString()).then(boost::bind(&RobloxTeamCreateWidget::updatePlaceDetails, this, _1));	
}

void RobloxTeamCreateWidget::updatePlaceDetails(ARL::HttpFuture future)
{
	typedef ARL::Reflection::ValueTable JsonTable;
...
void RobloxTeamCreateWidget::onFriendsInviteEditReturnPressed()
{
	...
	{
		ARL::StandardOut::singleton()->printf(ARL::MESSAGE_ERROR, "Requested creator '%s' already exists.", qPrintable(playerName));
		return;	
	}
	...
	prop.setFromJsonFuture(ARL::HttpAsync::get(QString("%1/users/get-by-username?username=%2").arg(RobloxSettings::getApiBaseURL()).arg(playerName).toStdString()));
	...
	{
		ARL::StandardOut::singleton()->printf(ARL::MESSAGE_ERROR, "Requested creator '%s' does not exist.", qPrintable(playerName));
	}
...
void RobloxTeamCreateWidget::onRemoveCloudEditConfirm()
{
	...
	if (playerId > 0 && removeCloudEditor(playerId))
	{
		...
		if (FFlag::KickUsersWhenTheyLoseAccessToCloudEdit)
		{
			...
			{
				ARL::DataModel::LegacyLock lock(m_pDataModel.get(), ARL::DataModelJob::Write);
				if (ARL::Network::Players* players = m_pDataModel->find<ARL::Network::Players>())
				{
					ARL::Network::Players::event_requestCloudEditKick.replicateEvent(players, playerId);
				}
			}
		}
	}
...
bool RobloxTeamCreateWidget::addCloudEditor(int playerId)
{
	...
	{		
		...
		{
			canAddEditor = false;
			ARL::StandardOut::singleton()->printf(ARL::MESSAGE_ERROR, "Error while turning on cloud edit");
		}
	}
...
bool RobloxTeamCreateWidget::canLaunchCloudEdit(int &universeId)
{
	...
	if (universeId <= 0)
	{
		...
		universeIdProp.setFromJsonFuture(ARL::HttpAsync::get(getUniverseUrl.toStdString()));
	...
	if (universeId > 0)
	{
		...
		cloudEditEnabledProp.setFromJsonFuture(ARL::HttpAsync::get(cloudEditEnabledUrl.toStdString()));
...
{	setDataModel(shared_ptr<ARL::DataModel>(), shared_ptr<PlayersDataManager>()); }
...
std::string RobloxTeamCreateWidget::doPost(const std::string& postUrl)
{
	...
	try
	{
		ARL::HttpPostData d("", ARL::Http::kContentTypeDefaultUnspecified, false);
		result = ARL::HttpAsync::post(postUrl, d).get();
	}
...
// FriendsInfoLoader
//--------------------------------------------------------------------------------------------
static void invokeFriendsDataLoaded(QPointer<FriendsInfoLoader> friendsInfoLoader, int pageLoaded, ARL::HttpFuture future)
{
	if (friendsInfoLoader.isNull())
		return;
	QMetaObject::invokeMethod(friendsInfoLoader.data(), "friendsDataLoaded", Qt::QueuedConnection, 
		Q_ARG(int, pageLoaded), Q_ARG(ARL::HttpFuture, future));
...
void FriendsInfoLoader::loadPage(int pageToLoad)
{
	...
	ARL::HttpAsync::get(str.toStdString()).then(boost::bind(&invokeFriendsDataLoaded, QPointer<FriendsInfoLoader>(this), pageToLoad, _1));
}

void FriendsInfoLoader::friendsDataLoaded(int pageLoaded, ARL::HttpFuture future)
{
	try 
	{
		boost::shared_ptr<const ARL::Reflection::ValueArray> table;
		if (!ARL::WebParser::parseJSONArray(future.get(), table) || table->size() <= 0)
		{
		...
		for (ARL::Reflection::ValueArray::const_iterator iter = table->begin(); iter != table->end(); ++iter)
		{
			if (iter->isType<shared_ptr<const ARL::Reflection::ValueTable> >())
			{
				friendsProp.setFromValueTable(iter->cast<shared_ptr<const ARL::Reflection::ValueTable> >());
...
// CreatorsListLoader
//--------------------------------------------------------------------------------------------
static void invokeCreatorsListLoaded(QPointer<CreatorsListLoader> creatorsListLoader, int pageLoaded, ARL::HttpFuture future)
{
...
void CreatorsListLoader::loadPage(int pageToLoad)
{
	...
	ARL::HttpAsync::get(str.toStdString()).then(boost::bind(&invokeCreatorsListLoaded, QPointer<CreatorsListLoader>(this), pageToLoad, _1));
}

void CreatorsListLoader::creatorsListLoaded(int pageLoaded, ARL::HttpFuture future)
{	
	typedef ARL::Reflection::ValueArray JsonArray;
	...
	if (creatorsList)
	{
		...
		for (JsonArray::const_iterator iter = creatorsList->begin(); iter != creatorsList->end(); ++iter)
		{
			if (iter->isType<shared_ptr<const ARL::Reflection::ValueTable> >())
			{
				creatorsDetails.setFromValueTable(iter->cast<shared_ptr<const ARL::Reflection::ValueTable> >());
