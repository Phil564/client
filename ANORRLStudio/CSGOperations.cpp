...
ARL::CSGOperations::CSGOperations( DataModel* pDatamodel, MessageCallback failedFun )
  : mDatamodel(pDatamodel)
  , operationFailed(failedFun)
{
}

bool ARL::CSGOperations::doUnion(
    std::vector<boost::shared_ptr<ARL::Instance> >::const_iterator start,
    std::vector<boost::shared_ptr<ARL::Instance> >::const_iterator end,
    boost::shared_ptr<PartOperation>& partOperation )
{
    bool allNeg = true;
    for (Instances::const_iterator iter = start; iter != end; ++iter)
    {
        if (!ARL::Instance::fastSharedDynamicCast<NegateOperation>(*iter))
        {
            allNeg = false;
            break;
        }
    ...
    if (!setUnionMesh(partOperation, start, end, false))
    {
        ...
        partOperation = boost::shared_ptr<ARL::PartOperation>();
        return false;
    }
	...
	if (FFlag::CSGDelayParentingOperationToEnd)
	{
		...
		for (auto iter : selectionCopy)
		{
			...
			if (shared_ptr<PartOperation> partOp = ARL::Instance::fastSharedDynamicCast<PartOperation>(iter))
			{
				dictionaryService->retrieveData(*partOp);
				nrDictionaryService->retrieveData(*partOp);

				partOp->setMeshData(ARL::BinaryString(""));
				partOp->setPhysicsData(ARL::BinaryString(""));
			}
		}
	}
	else
	{
		for (ARL::Instances::const_iterator iter = start; iter != end; ++iter)
		{
			(*iter)->setParent(partOperation.get());
    ...
    {
        char buf[128];
        sprintf( buf, "This union is too complex.  There is a %d triangle limit for unions.  Please undo the last change.", (int)ARL::PartOperation::getMaximumTriangleCount());
        partOperation->setParent(NULL);
        partOperation = boost::shared_ptr<ARL::PartOperation>();
	...
	if (FFlag::CSGDelayParentingOperationToEnd)
	{
		ARLASSERT(partOperation->getParent() == NULL);
		ARLASSERT(newOperationParent != NULL);
...
bool ARL::CSGOperations::doNegate(
    std::vector<boost::shared_ptr<ARL::Instance> >::const_iterator start,
    std::vector<boost::shared_ptr<ARL::Instance> >::const_iterator end,
    std::vector<shared_ptr<ARL::Instance> >& toSelect )
{
    std::vector<shared_ptr<ARL::Instance> > toRemove;

    bool success = negateSelection(toRemove, toSelect, start, end);

    if (!success)
    {
        for (std::vector<shared_ptr<ARL::Instance> >::iterator iter = toSelect.begin(); iter != toSelect.end(); ++iter)
            (*iter)->setParent(NULL);
        return true;
    }

    for (std::vector<shared_ptr<ARL::Instance> >::iterator iter = toRemove.begin(); iter != toRemove.end(); ++iter)
        (*iter)->setParent(NULL);

    return true;
}

bool ARL::CSGOperations::doSeparate(
    std::vector<boost::shared_ptr<ARL::Instance> >::const_iterator start,
    std::vector<boost::shared_ptr<ARL::Instance> >::const_iterator end,
    std::vector<shared_ptr<ARL::Instance> >& ungroupedItems )
{
    ...
    for (std::vector<boost::shared_ptr<ARL::Instance> >::const_iterator iter = start; iter != end; ++iter)
    {
...
bool ARL::CSGOperations::setOperationMesh(shared_ptr<PartOperation> partOperation, Instances instances)
{
...
bool ARL::CSGOperations::recalculateMesh(shared_ptr<PartOperation> partOperation)
{
...
bool ARL::CSGOperations::createPartMesh(PartInstance* part, CSGMesh* modelData)
{    
...
void ARL::CSGOperations::resetInstanceCFrame(
...
static void warnIfChildInstance(shared_ptr<ARL::Instance> instance,
                                shared_ptr<ARL::PartInstance> partInstance,
                                const std::string& operationType)
{
    if (instance->fastDynamicCast<ARL::DataModelMesh>())
        return;

    ARL::StandardOut::singleton()->printf(ARL::MESSAGE_ERROR, "Warning:  Child object \"%s\" of parent \"%s\" is not supported by %s.", instance->getFullName().c_str(), partInstance->getFullName().c_str(), operationType.c_str());
}

void ARL::CSGOperations::warnIfChildInstances(shared_ptr<PartInstance> partInstance, const std::string& operationType)
{
    partInstance->visitDescendants(boost::bind(&warnIfChildInstance, _1, partInstance, operationType));
}

const char* ARL::CSGOperations::unionFailedMsg = "This union is not currently solvable.  Try offseting the parts from each other or try unioning each part individually.";

bool ARL::CSGOperations::setUnionMesh(shared_ptr<PartOperation> finalPartOperation,
                                      std::vector<boost::shared_ptr<Instance> >::const_iterator start,
                                      std::vector<boost::shared_ptr<Instance> >::const_iterator end,
...
bool ARL::CSGOperations::setNegateMesh(shared_ptr<NegateOperation> negateOperation, shared_ptr<Instance> instance)
{
...
bool ARL::CSGOperations::negateSelection(Instances& toRemove,
                                         Instances& toSelect,
                                         std::vector<boost::shared_ptr<Instance> >::const_iterator start, 
                                         std::vector<boost::shared_ptr<Instance> >::const_iterator end)
{
...
bool ARL::CSGOperations::separate(const shared_ptr<Instance>& wInstance,
                                  Instances &separatedItems,
                                  Instances &itemsToDelete)
{
...
ARL::Instance* ARL::CSGOperations::getCommonParentOrWorkspace(Instances::const_iterator start,
