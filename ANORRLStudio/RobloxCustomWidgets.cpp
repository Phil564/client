...
, m_closeOnMouseRelease(false)
{
    ...
    m_colorsInfo.reserve(ARL::BrickColor::colorPalette().size());
	...
	for (int row = 0; row < numRows; row++)
		for (int col = 0; col < numCols; col++)
		{
			if((qAbs(numRows/2 - row) + qAbs(numCols/2 - col) < maxPolygonsInARow) && (row % 2 == col % 2))
			{
				...
				ARL::BrickColor bColor = ARL::BrickColor::colorPalette().operator[](numColorsUsed++);
	...
	static ARL::BrickColor::Number lowerLineColorIDs[12] = {ARL::BrickColor::brick_199, ARL::BrickColor::brick_194, ARL::BrickColor::roblox_1002, 
			                                                ARL::BrickColor::brick_325, ARL::BrickColor::brick_348, ARL::BrickColor::brick_26, 
															ARL::BrickColor::brick_302, ARL::BrickColor::brick_311, ARL::BrickColor::brick_320,  
															ARL::BrickColor::brick_335, ARL::BrickColor::roblox_1001, ARL::BrickColor::brick_1};

	int colorCount = ARL::BrickColor::colorPalette().size(), lowerLineColorCount = 0; 
	...
	for (int ii = 0; ii < numCols - 1; ++ii)
	{
		...
		ARL::BrickColor bColor;
		if (numColorsUsed < colorCount)
			bColor = ARL::BrickColor::colorPalette().operator[](numColorsUsed++);
		else 
			bColor = ARL::BrickColor(lowerLineColorIDs[lowerLineColorCount++]);
...
bool ColorPickerFrame::event(QEvent *evt)
{
	if (evt->type() == QEvent::ToolTip) 
	{
		...
		if (colorIndex >= 0)
		{
			ARL::BrickColor color(m_colorsInfo[colorIndex].colorValue);
...
{
	int colorCount = ARL::BrickColor::colorPalette().size();
	int temp = 3, result = 0;
	while (true)
	{
...
QPolygonF ColorPickerFrame::getPolygon(PolygonSize polySize)
{
	...
	ARLASSERT(polySize == PolygonSize_Normal);
...
{
	m_materialValues.append(ARL::PLASTIC_MATERIAL);
	m_materialValues.append(ARL::WOOD_MATERIAL);
	m_materialValues.append(ARL::WOODPLANKS_MATERIAL);
	m_materialValues.append(ARL::SLATE_MATERIAL);
	m_materialValues.append(ARL::CONCRETE_MATERIAL);
	m_materialValues.append(ARL::METAL_MATERIAL);
	m_materialValues.append(ARL::RUST_MATERIAL);
	m_materialValues.append(ARL::DIAMONDPLATE_MATERIAL);
	m_materialValues.append(ARL::ALUMINUM_MATERIAL);
	m_materialValues.append(ARL::GRASS_MATERIAL);
	m_materialValues.append(ARL::ICE_MATERIAL);
	m_materialValues.append(ARL::BRICK_MATERIAL);
	m_materialValues.append(ARL::SAND_MATERIAL);
	m_materialValues.append(ARL::FABRIC_MATERIAL);
	m_materialValues.append(ARL::GRANITE_MATERIAL);
	m_materialValues.append(ARL::MARBLE_MATERIAL);
	m_materialValues.append(ARL::PEBBLE_MATERIAL);
	m_materialValues.append(ARL::COBBLESTONE_MATERIAL);
	m_materialValues.append(ARL::SMOOTH_PLASTIC_MATERIAL);	
...
bool MaterialPickerFrame::event(QEvent *evt)
{
	if (evt->type() == QEvent::ToolTip) 
	{
		...
		if (materialIndex >= 0)
		{
			std::string materialName = ARL::Reflection::EnumDesc<ARL::PartMaterial>::singleton().convertToString((ARL::PartMaterial)m_materialValues[materialIndex]);
...
PickerFrame* ColorPickerToolButton::getPickerFrame()
{
	...
	m_pPickerFrame->setSelectedItem(ARL::ColorVerb::getCurrentColor().asInt());
...
PickerFrame* FillColorPickerToolButton::getPickerFrame()
{
	...
	{
		m_pPickerFrame->setSelectedItem(ARL::FillTool::color.get().asInt());
	}
...
QString ImagePickerWidget::getImageUrl(const QString& lastValue, const QPoint& location, boost::shared_ptr<ARL::DataModel> dm)
{
	...
	for (size_t i = 0; i < imageNames.size(); ++i)
	{
		...
		if (shared_ptr<const std::string> data =
			cp->requestContentString(ARL::ContentId(url.toStdString()),
				ARL::ContentProvider::PRIORITY_MFC))
		{
			QPixmap p;
...
PartSelectionTool::PartSelectionTool(ARL::Workspace* workspace)
	: ARL::Named<ARL::MouseCommand, sPartSelectionTool>(workspace)
	, m_CursorName("ArrowFarCursor")
	, m_bIsValidPartSelected(false)
{
}

void PartSelectionTool::onMouseHover(const shared_ptr<ARL::InputObject>& inputObject)
{
	bool isValid = false;
	ARL::PartInstance* pPartHoveredOver = getPart(workspace, inputObject);
	partHoveredOverSignal(pPartHoveredOver, isValid);

    setHighlightPart( isValid ? shared_from(pPartHoveredOver) : boost::shared_ptr<ARL::PartInstance>());
}

shared_ptr<ARL::MouseCommand> PartSelectionTool::onMouseDown(const shared_ptr<ARL::InputObject>& inputObject)
{
	// first emit signal
	...
	return shared_ptr<ARL::MouseCommand>();
}

shared_ptr<ARL::MouseCommand> PartSelectionTool::onMouseUp(const shared_ptr<ARL::InputObject>& inputObject)
{
	if (!m_bIsValidPartSelected)
		return shared_from(this);
	return shared_ptr<ARL::MouseCommand>();
...
void PartSelectionTool::render3dAdorn(ARL::Adorn* adorn)
{
	ARL::MouseCommand::render3dAdorn(adorn);
	if (m_pHighlightPart)
	{
	      ARL::Draw::selectionBox(m_pHighlightPart->getPart(), adorn, 
								  ARL::ModelInstance::primaryPartHoverOverColor(), 
								  ARL::ModelInstance::primaryPartLineThickness());
	}
}
...
//--------------------------------------------------------------------------------------------
PrimaryPartSelectionEditor::PrimaryPartSelectionEditor(QWidget* parent, PropertyItem* pPropertyItem, const QString& labelText, boost::shared_ptr<ARL::ModelInstance> instance)
: QWidget(parent)
...
, m_pModelInstance(instance)
{
	...
	ARL::Workspace* workspace = RobloxDocManager::Instance().getPlayDoc()->getDataModel()->getWorkspace();
	m_pSelectionTool = ARL::Creatable<ARL::MouseCommand>::create<PartSelectionTool>(workspace);
...
PrimaryPartSelectionEditor::~PrimaryPartSelectionEditor()
{
	...
	if (RobloxDocManager::Instance().getPlayDoc())
	{
		...
		{
			workspace->setMouseCommand(boost::shared_ptr<ARL::MouseCommand>());
...
void PrimaryPartSelectionEditor::onPartHoveredOver(ARL::PartInstance* pPart, bool& isValid)
{
...
void PrimaryPartSelectionEditor::onInstanceSelected(boost::shared_ptr<ARL::Instance> instance)
{  setPrimaryPart(ARL::Instance::fastDynamicCast<ARL::PartInstance>(instance.get())); }

void PrimaryPartSelectionEditor::onInstanceHovered(boost::shared_ptr<ARL::Instance> instance)
{
	bool isValid = false;
	onPartHoveredOver(ARL::Instance::fastDynamicCast<ARL::PartInstance>(instance.get()), isValid);
}

void PrimaryPartSelectionEditor::onPartSelected(ARL::PartInstance* pPart)
{
...
bool PrimaryPartSelectionEditor::setPrimaryPart(ARL::PartInstance* pPart)
{
	if (pPart && pPart->isDescendantOf(m_pModelInstance.get()))
	{
		if (pPart == m_pModelInstance->getPrimaryPartSetByUser())
		{
			ARL::StandardOut::singleton()->print(ARL::MESSAGE_ERROR, "Selected part is already set as primary part.");
			return false;
		}

		ARLASSERT(RobloxDocManager::Instance().getPlayDoc() && RobloxDocManager::Instance().getPlayDoc()->getDataModel());
		ARL::DataModel::LegacyLock lock(RobloxDocManager::Instance().getPlayDoc()->getDataModel(), ARL::DataModelJob::Write);
...
{
	QApplication::restoreOverrideCursor();
	QPixmap pixmap(ARL::ContentProvider::getAssetFile(cursorImage).c_str());
	if (!pixmap.isNull())
	{
