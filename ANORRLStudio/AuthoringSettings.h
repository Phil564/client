...
class AuthoringSettings : public ARL::GlobalAdvancedSettingsItem<AuthoringSettings, sAuthoringSettings>
{
public:
	...
	ARL::ChangeHistoryService::RuntimeUndoBehavior getRuntimeUndoBehavior() const { return ARL::ChangeHistoryService::runtimeUndoBehavior; }
	void setRuntimeUndoBehavior(ARL::ChangeHistoryService::RuntimeUndoBehavior value);

	ARL::Security::Permissions getPermissionShown() const;
	...
	bool isShown(ARL::Reflection::Metadata::Class* metadata, const DescriptorType& descriptor)
	{
		if (descriptor.security > getPermissionShown())
			return false;
		if (ARL::Reflection::Metadata::Item::isDeprecated(metadata, descriptor) && !showDeprecated)
			return false;
		if (!metadata)
			return false;
	...
	template<class DescriptorType>
	bool isHiddenInBrowser(ARL::Reflection::Metadata::Item* metadata, const DescriptorType& descriptor)
	{
		if (descriptor.security > getPermissionShown())
			return true;
		if (ARL::Reflection::Metadata::Item::isDeprecated(metadata, descriptor) && !showDeprecated)
	...
	void onPluginsDirChanged(const ARL::Reflection::PropertyDescriptor&);
